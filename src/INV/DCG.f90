...
type(dataVectorMTX_t)                      ::p_temp,JmHat
...
!  local variables
   type(dataVectorMTX_t)			:: dHat, b,dx,d_Pred,res,Nres,Jm0,d_Pred_m0
...
do DS_iter=1,5
	...
	! b= (d-dPred)+ J(m-m0)
	        ...
	        b=d
			write(*,*) "Norm JmHat: ", dotProd(JmHat,JmHat)
...
!530 continue	
   ...
   write(10,*)' Start Search for Lambda Full FWD',k_step  
...
beta=R_zero
 ...
 do ii=1,k_step
	  ...
	  do jj=1,nTx
	    ...
	    write(75,'(i5,3f10.3)')ii,beta(ii+1)/beta(1),beta(ii+1)/alpha,alpha/beta(ii+1)
        close(75)
!SVD on T        
