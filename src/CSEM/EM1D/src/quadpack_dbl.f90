...
subroutine qagi ( f, bound, inf, epsabs, epsrel, result, abserr, neval, ier, limit_in )
...
!
!> RS 05/2010: introduced limit_in to make max nr of subdivisions flexible
!
  implicit none

  integer, parameter :: limit_default = 500
  integer,optional :: limit_in
  integer :: limit
  integer :: ier0
  ...
  real*8 abserr
!!$  real*8 alist(limit)
  real*8,dimension(:),allocatable :: alist
  real*8 area
  real*8 area1
  real*8 area12
  real*8 area2
  ...
  real*8 a2
!!$  real*8 blist(limit)
  real*8,dimension(:),allocatable :: blist
  real*8 boun
  real*8 bound
  real*8 b1
  real*8 b2
...
!
  ...
  allocate(alist(limit),blist(limit),elist(limit),iord(limit),rlist(limit), stat=ier0)
...
!
  ...
  if( result /= R_ZEROd0 .AND. area /= R_ZEROd0) then
    go to 105
  endif
...
subroutine qagic ( f, fcplx, bound, inf, epsabs, epsrel, result, abserr, neval, ier )

!*****************************************************************************80
!
!> special version of qagi to allow for complex input function
!
!
!
  implicit none
...
!
    ...
    b2 = blist(maxerr)
    erlast = errmax
    call qk15ic ( f, fcplx, boun, inf, a1, b1, area1, error1, resabs, defab1 )
    call qk15ic ( f, fcplx, boun, inf, a2, b2, area2, error2, resabs, defab2 )
!
!>  Improve previous approximations to integral and error
!>  and test for accuracy.
!
...
!
  ...
  do i = 1, nint
    ...
    if( error1 == resa .AND. error1 /= R_ZEROd0 ) then
      ndin(i) = 1
    endif
...
!60 continue
  ...
  if( result /= R_ZEROd0 .AND. psum(numrl2) /= R_ZEROd0) go to 70

  if( abserr > errsum ) then
    go to 80
  endif

  if( psum(numrl2) == R_ZEROd0 ) then
    return
  endif
...
!
  amom0 = log ( abs ( ( 1.0d0 - cc ) / ( 1.0d0 + cc ) ) )
  amom1 = 2.0d0 + cc * amom0
...
!
  v(1) = 2.0d0 * sinpar / parint
  v(2) = (8.0d0*cospar+(par2+par2-8.0d0)*sinpar/ parint)/par2
  v(3) = (3.2e+01*(par2-1.2e+01)*cospar+(2.0d0* &
...
!
  ...
  do k = 1, noeq1
    an2 = an*an
    d(k) = -2.0d0*(an2-4.0d0) * (par22-an2-an2)
    d2(k) = (an-1.0d0)*(an-2.0d0) * par2
  ...
  v(noequ+3) = as - ( an2 - 4.0d0 ) * ac
  v(4) = v(4) - 5.6e+01 * par2 * v(3)
  ass = parint * sinpar
  asap = (((((2.10e+02*par2-1.0d0)*cospar-(1.05e+02*par2 &
     -6.3e+01)*ass)/an2-(1.0d0-1.5e+01*par2)*cospar &
     +1.5e+01*ass)/an2-cospar+3.0d0*ass)/an2-cospar)/an2
  v(noequ+3) = v(noequ+3)-2.0d0*asap*par2*(an-1.0d0)* &
      (an-2.0d0)
!
...
70 continue
  ...
  do i = 4, 13
    an2 = an*an
    v(i) = ((an2-4.0d0)*(2.0d0*(par22-an2-an2)*v(i-1)-ac) &
...
!
  v(1) = 2.0d0*(sinpar-parint*cospar)/par2
  v(2) = (1.8e+01-4.8e+01/par2)*sinpar/par2 &
     +(-2.0d0+4.8e+01/par2)*cospar/parint
...
!
    ...
    do i = 3, 12
      an2 = an*an
      v(i) = ((an2-4.0d0)*(2.0d0*(par22-an2-an2)*v(i-1)+as) &
       +ac-par2*(an+1.0d0)*(an+2.0d0)*v(i-2)) &
       /(par2*(an-1.0d0)*(an-2.0d0))
      an = an+2.0d0
      chebmo(m,2*i) = v(i)
    enddo
...
!
  ...
  if( bl == a .AND. (alfa /= R_ZEROd0 .OR. integr == 2 .OR. integr == 4)) then
    go to 10
  endif

  if( br == b .AND. (beta /= R_ZEROd0 .OR. integr == 3 .OR. integr == 4)) &
    go to 140
!
...
!
  ...
  if( ier == 0 ) ier = 3
  if( result /= R_ZEROd0.AND.area /= R_ZEROd0 ) go to 160
  if( abserr > errsum ) go to 170
  if( area == R_ZEROd0 ) go to 190
  go to 165
...
!
  ...
  if( resasc /= R_ZEROd0.AND.abserr /= R_ZEROd0 ) then
...
!
  data xgk(1),xgk(2),xgk(3),xgk(4),xgk(5),xgk(6),xgk(7),xgk(8)/ &
       ...
       8.648644233597691e-01,     7.415311855993944e-01, &
       5.860872354676911e-01,     4.058451513773972e-01, &
       2.077849550078985e-01,     0.0000000000000000d0/
  ...
  data wg(1),wg(2),wg(3),wg(4),wg(5),wg(6),wg(7),wg(8)/ &
       0.0000000000000000d0,     1.294849661688697e-01, &
       0.0000000000000000d0,     2.797053914892767e-01, &
       0.0000000000000000d0,     3.818300505051189e-01, &
       0.0000000000000000d0,     4.179591836734694e-01/
...
subroutine qk15ic ( f, fcplx, boun, inf, a, b, result, abserr, resabs, resasc )

!*****************************************************************************80
!> special version of qk15i for complex input function
!
!> we have f = f (fcplx,x)
!
  implicit none
...
!
  data xgk(1),xgk(2),xgk(3),xgk(4),xgk(5),xgk(6),xgk(7),xgk(8)/ &
       ...
       8.648644233597691e-01,     7.415311855993944e-01, &
       5.860872354676911e-01,     4.058451513773972e-01, &
       2.077849550789850e-01,     0.000000000000000d0/
...
!
 ...
 enddo
  ...
  if( resasc /= R_ZEROd0.AND.abserr /= R_ZEROd0) &
    abserr = resasc*min ( 1.0d0,(2.0e+02*abserr/resasc)**1.5d0)
