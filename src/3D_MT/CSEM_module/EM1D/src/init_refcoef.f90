...
if (refl_var%refcoef_changed) then
 ...
 isiso: if (aniso .eq. iso) then
  ...
  case (vmd)
    ...
    do ilay = 1,nlay-1
      rupallTE(ilay,1:fl) = (1._real64 - pvertall1(ilay,1:fl)/pvertall1(ilay+1,1:fl)) / &
              (1 + pvertall1(ilay,1:fl)/pvertall1(ilay+1,1:fl))
      if (pvertall1(ilay,298).eq.pvertall1(ilay+1,298)) then
        tupallTE(ilay,1:fl) = 1._real64
      else
        tupallTE(ilay,1:fl) = 2._real64*sqrt(pvertall1(ilay,1:fl)*pvertall1(ilay+1,1:fl)) / &
                (pvertall1(ilay+1,1:fl) + pvertall1(ilay,1:fl))
      endif
  ...
  case default
    ...
    do ilay = 1,nlay-1
      ...
      rupallTE(ilay,1:fl) = (1._real64 - pvertall1(ilay,1:fl)/pvertall1(ilay+1,1:fl)) / &
              (1._real64 + pvertall1(ilay,1:fl)/pvertall1(ilay+1,1:fl))
      if (pvertall1(ilay,298).eq.pvertall1(ilay+1,298)) then
        tupallTE(ilay,1:fl) = 1._real64
      else
        tupallTE(ilay,1:fl) = 2._real64*sqrt(pvertall1(ilay,1:fl)*pvertall1(ilay+1,1:fl)) / &
