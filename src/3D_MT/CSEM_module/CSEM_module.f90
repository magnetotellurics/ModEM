...
!Local
    ...
    write(*,*) 'Start using ', trim(compute_1D_from),' to solve the 1D FWD problem'
    if (trim(compute_1D_from)=="Dipole1D") then
       call  get_source_for_csem_Dipole1D(sigma,grid,iTx)
    elseif (trim(compute_1D_from)=="EM1D") then
       call get_source_for_csem_EM1D(sigma,grid,iTx)
    end if
	
    write(*,*) 'finish using ', trim(compute_1D_from),' to solve the 1D FWD problem'
...
!#########################################################################
subroutine get_source_for_csem_EM1D(sigma,grid,iTx)
	 type(modelParam_t),intent(in)		:: sigma
	 type(grid_t), intent(in)           :: grid 
	 integer, intent(in)                :: iTx
 ...
 call set1DModel(sigma,txDict(iTx)%xyzTx(1),txDict(iTx)%xyzTx(2))
...
subroutine create_background_data(grid,bgdat)  
 ...
 bgdat%rsplmin=50.0
   !bgdat%aniso = vti
   bgdat%aniso = vti !default isotropic, change to vti if any layer is VTI
   
    !write(*,*) 'bgdat%nlay', bgdat%nlay
   do ilay=1,bgdat%nlay
       
         if (ilay .gt. 19 .and. ilay .lt. 33) then  ! that was just for testing 
			bgdat%sigh(ilay) = sig1D_temp(ilay)
			bgdat%sigv(ilay) = sig1D_temp(ilay)
        ...
        !write(*,*) 'ilay= ', ilay, '  bgdat%sigh(ilay)= ',  bgdat%sigh(ilay)
        
   end do  
   do ilay=1,bgdat%nlay-1
        bgdat%zbound(ilay)=zlay1D_temp(ilay)
        !write(*,*) 'ilay= ', ilay, '   bgdat%zbound(ilay)= ',   bgdat%zbound(ilay)
   end do
...
subroutine get_source_for_csem_Dipole1D(sigma,grid,iTx)
