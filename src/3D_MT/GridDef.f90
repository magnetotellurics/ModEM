...
module GridDef
  ...
  type :: grid_t
     ...
     !**
     ! Williams - 
     !   Added for multiresolution grids
     integer :: NGrids
...
Contains
  ...
  type(grid_t),intent(inout)		:: gridOut
     ...
     !**
    ! Williams start - Multiresolution grid
    if (gridIn%NGrids > 0) then
       gridOut%NGrids = gridIn%NGrids
       if (allocated(gridOut%Cs)) deallocate(gridOut%Cs)
       allocate (gridOut%Cs(gridOut%NGrids))
       gridOut%Cs = gridIn%Cs
    end if
    ! Williams - End
