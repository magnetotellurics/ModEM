  ...
  function DataBlockHeader(txType,dataType) result (header)
    ...
    select case (dataType)
       ...
       case(Full_Impedance_wAzimuth)
          header = 'Period(s) Code GG_Lat GG_Lon X(m) Y(m) Z(m) Component Real Imag Error ExAzimuth'
       ...
       case(Exy_Ampli_Phase,Bxy_Ampli_Phase)
            ...
            select case (iDt)
                case(Bz_Ampli_Phase)
						do icomp = 1,ncomp
							...
							write(ioDat, '(a1)', iostat=ios,advance='no') ' '
							write(ioDat,'(a20,3f12.3)',iostat=ios,advance='no') trim(siteid),x(:)
							write(ioDat, '(a1)', iostat=ios,advance='no') ' '
							if (icomp .eq. 1) then
						       write(ioDat,'(a10,3es15.6)',iostat=ios) trim(compid),value(icomp),0.1
							else
							   write(ioDat,'(a10,3es15.6)',iostat=ios) trim(compid),value(icomp),5.0
							end if
...
!							error(2*icomp) = sqrt(value(2*icomp-1)*value(2*icomp-1) + value(2*icomp)*value(2*icomp))/100.0;
						...
						!
						!write( 1982, * ) "OUTPUT: ", conjugate, SI_factor, fileInfo(iTxt,iDt)%units_in_file, typeDict(iDt)%units
						!
                case(Full_Impedance,Off_Diagonal_Impedance,Full_Vertical_Components)
                ...
                case(Full_Impedance_wAzimuth)
                   Rx_Azi = rxDict(iRx)%Rx_Azi
                    do icomp = 1,ncomp/2
                        ...
                        if (conjugate) then
                            write(ioDat,'(a8,4es15.6)',iostat=ios) trim(compid),value(2*icomp-1),-value(2*icomp),error(2*icomp),Rx_Azi
                        else
                            write(ioDat,'(a8,4es15.6)',iostat=ios) trim(compid),value(2*icomp-1),value(2*icomp),error(2*icomp),Rx_Azi
                        end if
                        countData = countData + 1
...
! to read any kind of data, by adding a new case statement.
   ...
   subroutine read_Z_list(allData,cfile)
    ...
    READ_DATA_TYPE: do
        ...
        select case (iDt)
 			case(Bz_Ampli_Phase)
				read(ioDat,*,iostat=ios) Dipole, Period, Moment, Azi, Dip, Tx(1), Tx(2), Tx(3), code, x(1), x(2), x(3), compid, Zreal, Zerr
				if (ios /= 0 ) then
					backspace(ioDat)
					exit
				end if
                ...
                if (FindStr(gridCoords, CARTESIAN)>0) then
                    write(siteid,'(a20,2f9.3)') code,x(1),x(2)
			...
			case(Ex_Field,Ey_Field,Ez_Field,Bx_Field,By_Field,Bz_Field)
				...
				if (ios /= 0 ) then
					...
					exit
            			...
            			!
						!write( 1982, * ) "INPUT : ", conjugate, SI_factor, fileInfo(iTxt,iDt)%units_in_file, typeDict(iDt)%units
						!
            		    aTx%Tx_type='CSEM'
						aTx%nPol=1
            			aTx%Dipole = Dipole
            ...
            case(Full_Impedance_wAzimuth)
                read(ioDat,*,iostat=ios) Period,code,lat,lon,x(1),x(2),x(3),compid,Zreal,Zimag,Zerr,Rx_azi
