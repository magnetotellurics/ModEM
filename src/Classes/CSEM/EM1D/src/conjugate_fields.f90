...
!> for derivatives: apply factor to take deriv. with respect to ln(sigma0)
!
!> Rita Streich 2011
!---------------------------------------------------------
subroutine conjugate_fields(bgdat,refl_var,src)
  ...
  !internal variables
  complex(kind=real64) :: logfact            !factor for getting derivatives with respect to log(sigma0)
  ...
  if(bgdat%dowhat.ge.deriv) then
    if(src%type.EQ.dipole) then
      !apply factor for getting derivatives with respect to ln(sigma0) instead of complex epsilon
      ...
      do ilay=1,nlay
        logfact = conjg(dci * bgdat%sigh(ilay) / bgdat%omega)  !use sigma0 only, not complex conductivity!
