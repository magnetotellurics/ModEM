!>     ALGORITHM 588, COLLECTED ALGORITHMS FROM ACM.
!>     ALGORITHM APPEARED IN ACM-TRANS. MATH. SOFTWARE, VOL.8, NO. 4,
!>     DEC., 1982, P.369-370, P. 344-468.
SUBROUTINE HANKEL(BMAX,NB,NREL,TOL,NTOL,NORD,FUN1,IJREL,ZWORK,ZANS,ARG,NOFUN1,IERR)
  implicit none
!=======================================================================
      INTEGER(kind=int32) :: NB,NREL,NTOL,NORD(NREL),IJREL(2,NREL),NOFUN1,IERR
      REAL(kind=real32) :: BMAX,TOL,ARG(NB)
      COMPLEX(kind=real32) :: ZWORK(283,NREL),ZANS(NB,NREL)
!=======================================================================
!
!>  PURPOSE
!
!>     THE PURPOSE OF SUBPROGRAM HANKEL IS TO PROVIDE IN SINGLE PRECISION
...
!>    ADAPTIVE RELATED CONVOLUTION (SEE [2]), SUBPROGRAM HANKEL
!>    MINIMIZES EXTERNAL FUN1 CALLS (NOFUN1 AT EXIT) IN EVALUATING A
!>    TOTAL OF NB*NREL COMPLEX HANKEL TRANSFORMS OF ORDERS 0 AND (OR) 1,
...
!>    DOUBLE-PRECISION (TO CONSERVE STORAGE AND REDUCE ROUND-OFF),
!>    BUT ARE USED IN SINGLE-PRECISION IN THE COMPLEX FUNCTION FUN1.
!>    BOTH J0 AND J1 FILTER RESPONSE FUNCTIONS (WEIGHTS) WERE
!>    DESIGNED TO HAVE IDENTICAL ABSCISSA VALUES IN [2].  THE
!>    STORED J0,J1 FILTER WEIGHTS USED IN SUBPROGRAM HANKEL ARE TAKEN
!>    FROM [2].  MUCH OF THE LOGIC USED IN HANKEL FOLLOWS THE
!>    CODING USED IN [1] AND [2].  THE MAJOR DIFFERENCES ARE IN THE
!>    DEFINITION OF RELATED INPUT FUNCTIONS (SEE IJREL,ZWORK), AND
!>    FOR HANDLING OSCILLATING FUNCTIONS (SEE NTOL,ITOL).
!
!>      FOUR GENERAL CASES ARE POSSIBLE USING SUBPROGRAM HANKEL.
!
...
!
!>      THIS SUBPROGRAM WAS IMPLEMENTED AND TESTED ON A 32-BIT WORD
!>    MACHINE WITH EXP-RANGE APPROXIMATELY 10**-38 TO 10**+38 AND 24-BIT
!>    MANTISSA (ABOUT 7-DECIMAL DIGITS).  ONLY SINGLE-PRECISION
!>    COMPLEX ARITHMETIC IS USED.  DOUBLE-PRECISION REAL WORDS HAVE THE
!>    SAME EXP-RANGE, WITH A 56-BIT MANTISSA (ABOUT 16-DECIMAL DIGITS).
!>    FOR MACHINES WITH OTHER WORD SIZES, CHANGES IN THE NUMBER OF
...
!>    INPUT
!
!>      BMAX - INITIAL HANKEL TRANSFORM ARGUMENT B=BMAX>0.0 (ANY CASE),
!>             USED IN INTEGRAL FROM 0 TO INFINITY OF
!>             FUN1(G)*JN(G*B)*DG, WHERE JN=BESSEL FUNCTION OF ORDER N,
!>             N=0 OR 1, AND B>0.0. (SEE FUN1 DEFINITION BELOW).
...
!>             BMIN=BMAX*EXP(-.2*(NB-1)) DOES NOT UNDERFLOW THE EXPONENT
!>             RANGE.  THE B-LAGGED SPACING IS .2 IN LOG-SPACE. FOR
!>             CONVENIENCE IN SPLINE INTERPOLATION LATER, EACH B IN
!>             (BMIN,BMAX) IS RETURNED IN ARRAY ARG(I),I=1,NB, WHERE
!>             ARG(I+1)/ARG(I)=EXP(.2) FOR ALL I.  IF BMAX>BMIN>0 IS
!>             GIVEN, THEN AN EFFECTIVE VALUE OF NB IS DETERMINED AS
!>             NB=AINT(5.*ALOG(BMAX/BMIN))+I, WHERE I>1 IS RECOMMENDED,
...
!>             BEST TO USE ALOG(ARG(I)) INSTEAD OF ARG(I) -VS- ZANS(I,J),
!>             FOR I=1,NB, AND FOR ANY GIVEN J BETWEEN 1 AND NREL.  NOTE
!>             NB IS USED AS AN ADJUSTABLE DIMENSION IN ZANS(NB,NREL).
!>      NREL - NUMBER OF RELATED CONVOLUTIONS DESIRED (NREL.GE.1).  USE
!>             NREL=1 IF ONLY A SINGLE COMPLEX HANKEL TRANSFORM IS USED.
!>             NREL>1 REQUIRES ARRAY IJREL(2,NREL)  (SEE BELOW).
!>             NOTE NREL IS USED AS ADJUSTABLE DIMENSIONS IN ARRAYS
!>             ZANS(NB,NREL),ZWORK(283,NREL),NORD(NREL),IJREL(2,NREL).
!>       TOL - REQUESTED REAL TRUNCATION TOLERANCE AT BOTH FILTER TAILS
!>             FOR ADAPTIVE CONVOLUTION FOR ALL NB*NREL TRANSFORMS.  THE
!>             TRUNCATION CRITERION IS ESTABLISHED DURING CONVOLUTION IN
!>             A FIXED ABSCISSA RANGE (USING WEIGHTS 131-149) OF EITHER
...
!>             PRODUCT .LE. THE TRUNCATION CRITERION.  BOTH REAL AND
!>             IMAGINARY PARTS OF THE COMPLEX SUMMATION MUST SATISFY
!>             THE TRUNCATION CRITERION INDEPENDENTLY. IN GENERAL, A
...
!>             ONE MAY SET TOL=0.0 TO OBTAIN MAXIMUM ACCURACY FOR ALL
!>             NB*NREL COMPLEX HANKEL TRANSFORMS IN ZANS(NB,NREL).
!>             HOWEVER, THE ACTUAL RELATIVE ERRORS CANNOT BE EXPECTED
!>             TO BE SMALLER THAN ABOUT .1E-7 REGARDLESS OF THE TOLERANCE
!>             VALUE USED, SINCE SINGLE-PRECISION FILTER WEIGHTS AND
!>             SINGLE-PRECISION COMPLEX FUNCTIONS ARE USED. IN ANY EVENT,
...
!>      FUN1 - NAME OF AN EXTERNAL DECLARED COMPLEX FUNCTION OF A REAL
...
!>             COMPLEX FUNCTION FUN1(G) SUBPROGRAM MUST BE CODED BY THE
!>             USER AND MUST BE A CONTINUOUS DECREASING COMPLEX FUNCTION
!>             FOR ALL REAL G>0.0.  THE VALUE OF G MUST BE UNCHANGED
!>             UPON RETURN FROM FUN1. A MULTIPLE-POLE OF FUN1(G) AT G=0.0
!>             CAN EXIST, PROVIDED THE HANKEL TRANSFORM CONVERGES (NOTE
!>             FUN1(0.0) IS NOT USED). A SINGLE REAL FUNCTION F1(G) MAY
!>             BE CODED AS FUN1=complex(F1(G),0.0). TWO INDEPENDENT REAL
!>             FUNCTIONS F1(G),F2(G) MAY BE INTEGRATED IN PARALLEL BY
!>             CODING FUN1=complex(F1(G),F2(G)).  GENERALLY, FUN1(G)
!>             IS DEFINED ANALYTICALLY FOR ALL G>0.0.  HOWEVER,
...
!>             FILTER NYQUIST FREQUENCY, 1/(2*0.2).  IN GENERAL,
!>             SUBPROGRAM HANKEL PERFORMS BEST WHEN USING SMOOTH, WELL-
...
!>             COMPLEX FUNCTION. MORE COMPLICATED CODE COULD BE USED FOR
!>             THE RELATED FUNCTIONS, PROVIDED THE MEANING OF IJREL(2,K)
...
!>     ZWORK - COMPLEX WORK ARRAY ZWORK(283,NREL), WHICH IS USED TO HOLD
!>             VARIOUS COMPUTED FUNCTIONAL VALUES DURING RELATED AND
!>             LAGGED CONVOLUTIONS.  A STORAGE ROLL FEATURE USING
!>             ZWORK(283,NREL) AND INTERNAL ARRAY KEY(283) ALLOWS FOR
!>             ANY B RANGE (BMIN,BMAX) TO BE USED DURING CONVOLUTION.
!
!>  OUTPUT
!
!>      ZANS - THE COMPLEX ARRAY ZANS(NB,NREL) IS RETURNED GIVING THE
!>             NB*NREL COMPLEX HANKEL TRANSFORMS, WITH CORRESPONDING
!>             B ARGUMENTS GIVEN IN REAL ARRAY ARG(NB).
!>       ARG - REAL ARRAY ARG(NB) IS RETURNED GIVING THE RESULTING
!>             B ARGUMENTS IN (BMIN,BMAX), WHERE ARG(I+1)/ARG(I)=EXP(.2),
!>             I=1,NB-1 (THIS ARRAY COULD BE ELIMINATED TO SAVE STORAGE
!>             AND REGENERATED AFTER THE CALL HANKEL, IF DESIRED).
!>    NOFUN1 - NUMBER OF DIRECT FUN1 EVALUATIONS USED FOR ALL NB*NREL
!>             COMPLEX HANKEL TRANSFORMS.  NOFUN1 IS USUALLY NOT MORE
...
!>             = 0, NO ERROR IN INPUT PARAMETERS. ZANS,ARG COMPUTED.
!>             = 1, IMPROPER INPUT PARAMETERS (I.E., NB<1,NREL<1,BMAX<=0,
!>               OR BMAX*EXP(-.2*(NB-1))<=0.0).  ZANS,ARG NOT COMPUTED.
!
!
!>  ERROR CONDITIONS
!
...
!>         A VALUE OF 0.0 FOR THE COMPUTER SYSTEM BEING USED.  NOTE THAT
!>         UNDERFLOW MAY ALSO OCCUR IN THE USERS EXTERNAL FUNCTION
!>         FUN1(G) FOR ANY VALUE OF G AS SET BY SUBPROGRAM HANKEL.
...
!>         DOES NOT YIELD SINGLE-PRECISION COMPLEX ACCURACY, OR IS
!>         NOT A CONTINUOUS DECREASING COMPLEX FUNCTION FOR ALL G>0.
!
!>  USAGE
!
!>    SUBPROGRAM HANKEL IS CALLED AS FOLLOWS (USE NUMERICAL VALUES FOR
!>    <EXPRESSION>, EXCLUDING < AND >, IN DECLARATIONS) --
!
!>      COMPLEX ZANS(<NB>,<NREL> -OR- <NB*NREL>),ZWORK(283,<NREL>)
!>      DIMENSION ARG(<NB>),NORD(<NREL>),IJREL(2,<NREL>)
!>      EXTERNAL ZFUN1
!C-----READ/LOAD INPUT PARAMETERS FOR HANKEL AS REQUIRED
!>      ...
!>      CALL HANKEL(BMAX,NB,NREL,TOL,NTOL,NORD,ZFUN1,IJREL,ZWORK,
!>     * ZANS,ARG,NOFUN1,IERR)
!>      IF(IERR.EQ.1) STOP
!>      ...
!>      END
!>      COMPLEX FUNCTION ZFUN1(G)
!C-----INSERT USER SUPPLIED CODE FOR EVALUATION OF ZFUN1(G),G>0.0
!>      END
!
!=======================================================================
      COMPLEX(kind=real32) :: C,CMAX,ZSUM,ZERO,FUN1
      INTEGER(kind=int32) :: KEY(283)
      real(kind=real64) :: E,ER,Y1,Y,ABSCIS
      REAL(kind=real32) :: T,TMAX
      DIMENSION T(2),TMAX(2)
      REAL(kind=real32) :: WT0,WT1
      DIMENSION WT0(283),WT1(283)
!> RS: explicitly define all variables
      INTEGER(kind=int32) :: I
      INTEGER(kind=int32) :: NB1,LAG,ILAG,ISTORE,JREL,NONE,ITOL,M,LOOK,IQ,IR,IROLL,J
      REAL(kind=real32) :: G

!-----WE DEFINE COMPLEX C,CMAX TO BE EQUIVALENT TO REAL ARRAYS T(2),
!>     TMAX(2), RESPECTIVELY, FOR USE IN THE TRUNCATION CRITERION TESTS,
...
!-----E=DEXP(.2D0), ER=1.0D0/E (ALSO USED IN ABSCISSA GENERATION)
      DATA E,ER/1.221402758160169834D0,0.818730753077981859D0/
!-----WT0(I)=J0 HANKEL TRANSFORM FILTER WEIGHTS FOR I=1,283
      DATA &
...
!-----WT1(I)=J1 HANKEL TRANSFORM FILTER WEIGHTS FOR I=1,283
      DATA &
...
!-----INITIALIZE RELATED CONVOLUTION WITHIN LAGGED CONVOLUTION LOOPS
   ...
   10   KEY(I)=0
      NB1=NB+1
      LAG=-1
!-----PRESET INITIAL FILTER ABSCISSA FOR STARTING BMAX, GENERATED IN
!>     DOUBLE-PRECISION (TO REDUCE ROUND-OFF), BUT USED IN SINGLE-
!>     PRECISION IN THE COMPLEX FUNCTION FUN1(G).  NOTE THE ABSCISSAS
!>     ARE EQUALLY SPACED (E=DEXP(.2D0), ER=1.0D0/E) IN LOG-SPACE.
      Y1=ABSCIS/DBLE(BMAX)
...
!>         FILTER FIXED RANGE (USING WEIGHTS 131-149).
          NONE=0
          ITOL=NTOL
          ZSUM=ZERO
          CMAX=ZERO
          Y=Y1
!---------BEGIN RIGHT SIDE CONVOLUTION AT WEIGHT 131 (M=RETURN LABEL)
          ASSIGN 20 TO M
          I=131
          Y=Y*E
...
!---------ESTABLISH TRUNCATION CRITERION (CMAX=complex(TMAX(1),TMAX(2))
          CMAX=TOL*CMAX
          ASSIGN 30 TO M
...
!---------CHECK FOR FILTER TRUNCATION AT LEFT END
   60     IF(ABS(T(1)).LE.TMAX(1).AND.ABS(T(2)).LE.TMAX(2).AND.NONE.EQ.0) GO TO 80
          ITOL=NTOL
   70     I=I-1
          Y=Y*ER
!---------CALL PSEUDO SUBROUTINE AT 100 (RETURN TO 60 VIA M ASSIGNED)
...
!---------NORMALIZE ZSUM BY ARG(ISTORE) TO ACCOUNT FOR INTEGRATION
!>         RANGE CHANGE, AND STORE IN ZANS(ISTORE,JREL)
...
!>     A MAXIMUM OF 283*NB*NREL TIMES, WHERE PARAMETERS NB>0 AND NREL>0
!>     CAN BE ARBITRARILY LARGE.  IF A MORE-STRUCTURED STANDARD FORTRAN
!>     SUBROUTINE CALL WAS USED, THEN THE USUAL COMPILER LINKAGE
!>     CONVENTION COULD GENERATE A MAXIMUM OF 283*NB*NREL MACHINE-
...
!=========USE EXISTING SAVED FUNCTIONAL VALUES IN ZWORK(IR,JREL)
...
!>         THE LAST CONVOLUTION PRODUCT C=complex(T(1),T(2)).
          GO TO M,(20,30,60)
...
!=========FILL-IN REMAINING RELATED ZWORK(IR,J),J=2,NREL FOR THIS IR
          DO 160 J=2,NREL
!***********FOR OTHER THAN SIMPLE RELATIONS, THE FOLLOWING STATEMENT
!>           COULD BE CHANGED (AND ALSO THE MEANING OF IJREL(2,NREL)).
  160       ZWORK(IR,J)=complex(G**IJREL(1,J),0.0)*ZWORK(IR,1)**IJREL(2,J)
          GO TO 110
...
!-----END LOOP 1010 (GET REMAINING LAGGED CONVOLUTIONS FOR NEXT ARG)
 1010 CONTINUE
!-----EXIT WITH ZANS(NB,NREL),ARG(NB) COMPLETED WITH MINIMAL FUN1 CALLS
      RETURN
 9999 IERR=1
      RETURN
...
!
!
SUBROUTINE DHANKL(BMAX,NB,NREL,TOL,NTOL,NORD,FUN1,IJREL,DWORK,DANS,ARG,NOFUN1,IERR)
  implicit none
!=======================================================================
      INTEGER(kind=int32) :: NB,NREL,NTOL,NORD(NREL),IJREL(2,NREL),NOFUN1,IERR
      real(kind=real64) :: BMAX,TOL,DWORK(801,NREL),DANS(NB,NREL),ARG(NB)
!=======================================================================
!
!>  PURPOSE
!
...
!>      EXTERNAL DFUN1
!C-----READ/LOAD INPUT PARAMETERS FOR DHANKL AS REQUIRED
!>      ...
...
!>      DOUBLE PRECISION G
!C-----INSERT USER SUPPLIED CODE FOR EVALUATION OF DFUN1(G),G>0.0D0
!>      END
!
!=======================================================================
      real(kind=real64) :: ABSCIS,C,CMAX,DSUM,E,ER,FUN1,G,Y,Y1,WT0,WT1
      INTEGER(kind=int32) :: KEY
      DIMENSION KEY(801),WT0(801),WT1(801)
!> RS: explicitly define all variables
      INTEGER(kind=int32) :: I,NB1,LAG,ILAG,ISTORE,JREL,NONE,ITOL,M,LOOK,IQ,IR,IROLL,J
...
!-----EXIT WITH DANS(NB,NREL),ARG(NB) COMPLETED WITH MINIMAL FUN1 CALLS
 ...
 9999 IERR=1
      RETURN
 ENDSUBROUTINE DHANKL
!
!
!
SUBROUTINE zhankl(BMAX,NB,NREL,TOL,NTOL,NORD,FUN1,IJREL,ZWORK,ZANS,ARG,NOFUN1,IERR)
  implicit none
!=======================================================================
      INTEGER(kind=int32),intent( in ) :: NB,NREL,NTOL,NORD(NREL),IJREL(2,NREL)
      integer(kind=int32),intent( out ) :: NOFUN1,IERR
      REAL(kind=real64),intent( in ) :: BMAX,TOL
      REAL(kind=real64),dimension(NB) :: ARG
      COMPLEX(kind=real64),dimension(801,NREL),intent( out ) :: ZWORK
      COMPLEX(kind=real64),dimension(NB,NREL),intent( out ) :: ZANS
!=======================================================================
!
!>  RS 26.05.2009: complex double precision version of HANKEL
!
!=======================================================================
      real(kind=real64) :: G
      COMPLEX(kind=real64) :: C,CMAX,ZSUM,ZERO,FUN1
      INTEGER(kind=int32),dimension(801) :: KEY
      real(kind=real64) :: E,ER,Y1,Y,ABSCIS
      REAL(kind=real64),dimension(2) :: T,TMAX
      REAL(kind=real64),dimension(801) :: WT0,WT1
!> RS: explicitly define all variables
      INTEGER(kind=int32) :: I
      INTEGER(kind=int32) :: NB1,LAG,ILAG,ISTORE,JREL,NONE,ITOL,M,LOOK,IQ,IR,IROLL,J
...
!---------CALL PSEUDO SUBROUTINE AT 100 (RETURN TO 20 VIA M ASSIGNED)
          GO TO 100
   20     TMAX(1)=MAX(ABS(T(1)),TMAX(1))
          TMAX(2)=MAX(ABS(T(2)),TMAX(2))
          I=I+1
          Y=Y*E
!---------CALL PSEUDO SUBROUTINE AT 100 (RETURN TO 20 VIA M ASSIGNED)
          IF(I.LE.339) GO TO 100
          IF(TMAX(1).EQ.0.0D0.AND.TMAX(2).EQ.0.0D0) NONE=1
!---------ESTABLISH TRUNCATION CRITERION (CMAX=complex(TMAX(1),TMAX(2))
          CMAX=TOL*CMAX
          ASSIGN 30 TO M
!---------CALL PSEUDO SUBROUTINE AT 100 (RETURN TO 30 VIA M ASSIGNED)
...
!=========COMPUTE EXTERNAL FUN1 DIRECTLY ONLY WHEN NECESSARY
  150     KEY(IR)=IROLL+IR
          G=Y
!RS: make wavenumber counter visible outside hankel routine
          ikap = I + LAG
          ZWORK(IR,1)=FUN1(G)
          NOFUN1=NOFUN1+1
          IF(NREL.EQ.1) GO TO 110
...
!***********FOR OTHER THAN SIMPLE RELATIONS, THE FOLLOWING STATEMENT
!>           COULD BE CHANGED (AND ALSO THE MEANING OF IJREL(2,NREL)).
  160       ZWORK(IR,J)=G**IJREL(1,J)*ZWORK(IR,1)**IJREL(2,J)
          GO TO 110
!=========END OF PSEUDO SUBROUTINE (ENTRY 100, RETURN GO TO M ABOVE)
!=======================================================================
!
