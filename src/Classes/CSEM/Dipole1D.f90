...
!
    ...
    implicit none       
!
! used for finite dipole calculations
!
...
!
    ...
    cc = cos(ang)
    ss = sin(ang)
    ex1D(i) = exh*cc + exv*ss !> save fields for site i inoutput arrays
...
!
    do i = 2,iTxlayer
      ...
      if(linversion) then
            ...
            !> Post-apply expmgh(i-1) to  all recursion coefficients (done to avoid exp(+) term in final formula):
            dSmdsig(1:i-1,i-1) = dSmdsig(1:i-1,i-1)*expmgh(i-1) !> note that rows are now perturbed sigma0, 
