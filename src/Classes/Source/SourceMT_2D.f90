...
contains
   ...
   subroutine setESourceMT_2D( self, omega, polarization )
     ...
     if( polarization == 1 ) then
        !
        n_slice = self%model_operator%grid%ny
        !
        grid_2D = Grid2D_t( self%model_operator%grid%ny, &
        self%model_operator%grid%nzAir, self%model_operator%grid%nzEarth, &
        self%model_operator%grid%dy, self%model_operator%grid%dz )
     !
     ! Ey-polarization
     else
        !
        n_slice = self%model_operator%grid%nx
        !
        grid_2D = Grid2D_t( self%model_operator%grid%nx, &
        self%model_operator%grid%nzAir, self%model_operator%grid%nzEarth, &
        self%model_operator%grid%dx, self%model_operator%grid%dz )
        !
     endif
     !
     !
