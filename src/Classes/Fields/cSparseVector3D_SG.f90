!
!> Lone class to define a cSparseVector3D_SG
!
module cSparseVector3D_SG
    ...
    type :: cSparseVector3D_SG_t
        !
        class( Grid_t ), pointer :: grid        !Field property
        !
        character( len=4 ) :: grid_type         !Field property
        !
        integer :: nCoeff
        !
        ...
        !
        logical :: is_allocated                 !Field property
        !
        contains
            !
            final :: cSparseVector3D_SG_dtor
            !
            procedure, public :: multByValue => multByValueVectorcSparseVector3D_SG
            generic :: mult => multByValue
            !
...
contains
    ...
    function dotProdcSparseVector3D_SG( self, cvector ) result( cvalue )
        ...
        if( .NOT. cvector%is_allocated ) then
            stop "cvector not is_allocated yet for dotProdSparse"
        endif
        !
    ...
    !
    !> No function briefing
    subroutine multByValueVectorcSparseVector3D_SG( self, complex_value )
        implicit none
        !
        class( cSparseVector3D_SG_t ), intent( inout ) :: self
        !
        complex( kind=prec ), intent( in ) :: complex_value
        !
        integer :: ii
        !
        do ii = 1, size( self%xyz )
            self%c(ii) = self%c(ii) * complex_value
        enddo
        !
        self%is_allocated = .TRUE.
        !
    end subroutine multByValueVectorcSparseVector3D_SG
    !
    !> No function briefing
    ...
    !
    !> No subroutine briefing
    subroutine printcSparseVector3D_SG( self, io_unit, title )
        implicit none
        !
        class( cSparseVector3D_SG_t ), intent( in ) :: self
