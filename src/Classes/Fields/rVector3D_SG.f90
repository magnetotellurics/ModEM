...
contains
    ...
    function rVector3D_SG_ctor(igrid, gridType) result (self)
        ...
        if (self%gridType == EDGE) then
            allocate(self%x(nx, ny + 1, nz + 1), STAT = status)
            self%is_allocated = status.eq.0
            
            allocate(self%y(nx + 1, ny, nz + 1), STAT = status)
            self%is_allocated = self%is_allocated.and.(status.eq.0)
            
            allocate(self%z(nx + 1, ny + 1, nz), STAT = status)
            self%is_allocated = self%is_allocated.and.(status.eq.0)
            ...
            allocate(self%x(nx + 1, ny, nz), STAT = status)
            self%is_allocated = status.eq.0
            
            allocate(self%y(nx, ny + 1, nz), STAT = status)
            self%is_allocated = self%is_allocated.and.(status.eq.0)
            
            allocate(self%z(nx, ny, nz + 1), STAT = status)
            self%is_allocated = self%is_allocated.and.(status.eq.0)
    ...
    subroutine rVector3D_SG_dtor(self)
        implicit none
        !
        type( rVector3D_SG_t ), intent( in out ) :: self
        !
