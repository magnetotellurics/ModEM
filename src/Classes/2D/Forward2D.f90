...
module Forward2D
  ...
  implicit none
   ...
   type  :: Forward2D_t
      class(ModelParameter2D_t), pointer  :: m => null()
   ...
   contains
     ...
     ! procedure, public  :: SetEquations !  just do this as part of creation
      procedure, public  :: createCC
 ...
 contains
   ...
   function Forward2D_ctor(m) result( self )
      ...
      call createCC( self )   !  create curl-curl operator
      !
	  call self%setCond( m )
	  !
   ...
   subroutine Solve( self, E2D )
      ...
      if(.not.E2D%isAllocated) then
        print*,'Error in Forward2D%Solve: Esoln2D object not allocated'
        stop
      endif
      !
