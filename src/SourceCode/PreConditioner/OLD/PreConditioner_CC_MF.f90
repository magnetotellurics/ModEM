...
module PreConditioner_CC_MF
    ...
    type, extends( PreConditioner_t ) :: PreConditioner_CC_MF_t
        ...
        contains
            ...
            !
            procedure, public :: LTSolve => LTSolve_PreConditioner_CC_MF !> These are left (M1) and right (M2)
            procedure, public :: UTSolve => UTSolve_PreConditioner_CC_MF !> preconditioning matrices for curl-curl equation.
            procedure, public :: LUSolve => LUSolve_PreConditioner_CC_MF !> preconditoner for symmetric divCGrad operator
            !
...
contains
    ...
    subroutine setPreConditioner_CC_MF( self, omega )
        ...
        !
        write( *, * ) "setPreConditioner_CC_MF : ", omega
        !
    ...
    !
    !> Procedure LTSolve_PreConditioner_CC_MF
    !> Purpose: to solve the lower triangular system (or it"s adjoint);
    !> for the d-ilu pre-conditioner.
    !
    subroutine LTSolve_PreConditioner_CC_MF( self, in_e, out_e, adjoint )
        implicit none
        !
        class( PreConditioner_CC_MF_t ), intent( inout ) :: self
        ...
        complex( kind=prec ), allocatable, dimension(:, :, :) :: dilu_x, dilu_y, dilu_z
		!
		write(*,*) "LTSolve_PreConditioner_CC_MF: ", in_e%length(), out_e%length(), adjoint
		!
        if( .NOT. in_e%is_allocated ) then
            call errStop( "LTSolve_PreConditioner_CC_MF > in_e not allocated yet" )
        endif
        !
        in_e_x = in_e%getX()
        in_e_y = in_e%getY()
        ...
        if( .NOT. out_e%is_allocated ) then
            call errStop( "LTSolve_PreConditioner_CC_MF > out_e not allocated yet" )
        endif
        !
        !> Instantiate the ModelOperator object
        select type( model_operator => self%model_operator )
            ...
            class is( ModelOperator_MF_SG_t )
                ...
                !
            class default
                call errStop( "LTSolve_PreConditioner_CC_MF > Unclassified ModelOperator" )
            !
        end select
        !
    end subroutine LTSolve_PreConditioner_CC_MF
    !
    !> Procedure UTSolve_PreConditioner_CC_MF
    !> Purpose: to solve the upper triangular system (or it"s adjoint);
    !> for the d-ilu pre-condtioner
    !
    subroutine UTSolve_PreConditioner_CC_MF( self, in_e, out_e, adjoint )
        implicit none
        !
        class( PreConditioner_CC_MF_t ), intent( inout ) :: self
        class( Vector_t ), intent( inout ) :: in_e, out_e
        ...
        complex( kind=prec ), allocatable, dimension(:, :, :) :: dilu_x, dilu_y, dilu_z
		!
		write(*,*) "UTSolve_PreConditioner_CC_MF: ", in_e%length(), out_e%length(), adjoint
		!
        in_e_x = in_e%getX()
        in_e_y = in_e%getY()
        in_e_z = in_e%getZ()
        ...
        select type( model_operator => self%model_operator )
            ...
            class is( ModelOperator_MF_SG_t )
                ...
                !
            class default
                call errStop( "UTSolve_PreConditioner_CC_MF > Unclassified ModelOperator" )
            !
        end select
        !
    end subroutine UTSolve_PreConditioner_CC_MF
    !
    !> Procedure LUSolve_PreConditioner_CC_MF
    !> this is dummy routine required by abstract preconditioner class
    subroutine LUSolve_PreConditioner_CC_MF( self, in_phi, out_phi )
        implicit none
        !
        class( PreConditioner_CC_MF_t ), intent( inout ) :: self
        ...
        !
        call errStop( "LUSolve_PreConditioner_CC_MF not implemented" )
        !
    end subroutine LUSolve_PreConditioner_CC_MF
    !
