...
module cVector3D_SG
    ...
    type, extends( Vector_t ) :: cVector3D_SG_t
        ...
        contains
            ...
            !
            procedure, public :: sumCell => sumCell_cVector3D_SG
            procedure, public :: sumCellVTI => sumCellVTI_cVector3D_SG
            !
...
contains
    ...
    subroutine sumEdge_cVector3D_SG( self, cell_out, interior_only )
        ...
        !
        allocate( cell_out, source = cScalar3D_SG_t( self%grid, CELL ) )
        !
    ...
    subroutine sumEdgeVTI_cVector3D_SG( self, cell_h_out, cell_v_out, interior_only )
        ...
        !
        allocate( cell_h_out, source = cScalar3D_SG_t( self%grid, CELL ) )
        !
        allocate( cell_v_out, source = cScalar3D_SG_t( self%grid, CELL ) )
        !
    ...
    !> No subroutine briefing
    !
    subroutine sumCell_cVector3D_SG( self, cell_in, ptype )
        implicit none
        !
        class( cVector3D_SG_t ), intent( inout ) :: self
    ...
    !> No subroutine briefing
    !
    subroutine sumCellVTI_cVector3D_SG( self, cell_h_in, cell_v_in, ptype )
        implicit none
        !
        class( cVector3D_SG_t ), intent( inout ) :: self
        ...
        !
        if( .NOT. self%is_allocated ) then
             call errStop( "sumCellVTI_cVector3D_SG > self not allocated." )
        endif
        !
        if( .NOT. cell_h_in%is_allocated ) then
             call errStop( "sumCellVTI_cVector3D_SG > cell_h_in not allocated." )
        endif
        !
    ...
    subroutine add_cVector3D_SG( self, rhs )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "add_cVector3D_SG > self not allocated." )
        endif
        !
        if( .NOT. rhs%is_allocated ) then
            call errStop( "add_cVector3D_SG > rhs not allocated." )
        endif
        !
        if( self%isCompatible( rhs ) ) then
            ...
            select type( rhs )
                ...
                class is( rVector3D_SG_t )
                    ...
                    self%z = self%z + rhs%z
                    !
                    call errStop( "add_cVector3D_SG > rhs is rVector3D_SG_t" )
                    !
                class default
    ...
    subroutine subField_cVector3D_SG( self, rhs )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "subField_cVector3D_SG > self not allocated." )
        endif
        !
        if( .NOT. rhs%is_allocated ) then
            call errStop( "subField_cVector3D_SG > rhs not allocated." )
        endif
        !
        if( self%isCompatible( rhs ) ) then
            !
            select type( rhs )
                ...
                class is( rVector3D_SG_t )
                    ...
                    self%z = self%z - rhs%z
                    !
                    call errStop( "subField_cVector3D_SG > rhs is rVector3D_SG_t" )
                    !
                class default
    ...
    subroutine linComb_cVector3D_SG( self, rhs, c1, c2 )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "linComb_cVector3D_SG > self not allocated." )
        endif
        !
        if( .NOT. rhs%is_allocated ) then
            call errStop( "linComb_cVector3D_SG > rhs not allocated." )
        endif
        !
        if( self%isCompatible( rhs ) ) then
            !
            select type( rhs )
                ...
                class is( rVector3D_SG_t )
                    ...
                    self%z = c1 * self%z + c2 * rhs%z
                    !
                    call errStop( "linComb_cVector3D_SG > rhs is rVector3D_SG_t" )
                    !
                class default
    ...
    subroutine multByReal_cVector3D_SG( self, rvalue )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "multByReal_cVector3D_SG > self not allocated." )
        endif
        !
    ...
    subroutine multByComplex_cVector3D_SG( self, cvalue )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "multByComplex_cVector3D_SG > self not allocated." )
        endif
        !
        self%x = self%x * cvalue
        self%y = self%y * cvalue
    ...
    subroutine multByField_cVector3D_SG( self, rhs )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "multByField_cVector3D_SG > self not allocated." )
        endif
        !
        if( .NOT. rhs%is_allocated ) then
            call errStop( "multByField_cVector3D_SG > rhs not allocated." )
        endif
        !
        if( self%isCompatible( rhs ) ) then
            !
            ...
            call errStop( "multByField_cVector3D_SG > Incompatible inputs." )
        endif
        !
    ...
    function diagMult_cVector3D_SG( self, rhs ) result( diag_mult )
        ...
        !
        type( cVector3D_SG_t ) :: diag_mult_temp
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "diagMult_cVector3D_SG > self not allocated." )
        endif
        !
        if( .NOT. rhs%is_allocated ) then
            call errStop( "diagMult_cVector3D_SG > rhs not allocated." )
        endif
        !
        if( self%isCompatible( rhs ) ) then
            !
            ...
            select type( rhs )
                ...
                class is( rVector3D_SG_t )
                    ...
                    diag_mult_temp%z = self%z * rhs%z
                    !
                    call errStop( "diagMult_cVector3D_SG > rhs is rVector3D_SG_t" )
                    !
                class default
    ...
    subroutine multAdd_cVector3D_SG( self, cvalue, rhs )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "multAdd_cVector3D_SG > self not allocated." )
        endif
        !
        if( .NOT. rhs%is_allocated ) then
            call errStop( "multAdd_cVector3D_SG > rhs not allocated." )
        endif
        !
        if( self%isCompatible( rhs ) ) then
            !
            select type( rhs )
                ...
                class is( rVector3D_SG_t )
                    ...
                    self%z = self%z + cvalue * rhs%z
                    !
                    call errStop( "multAdd_cVector3D_SG > rhs is rVector3D_SG_t" )
                    !
                class default
    ...
    function dotProd_cVector3D_SG( self, rhs ) result( cvalue )
        ...
        if( self%isCompatible( rhs ) ) then
            ...
            call errStop( "dotProd_cVector3D_SG > Incompatible inputs." )
        endif
        !
    ...
    subroutine divByField_cVector3D_SG( self, rhs )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "divByField_cVector3D_SG > self not allocated." )
        endif
        !
        if( .NOT. rhs%is_allocated ) then
            call errStop( "divByField_cVector3D_SG > rhs not allocated." )
        endif
        !
        if( self%isCompatible( rhs ) ) then
            !
            ...
            call errStop( "divByField_cVector3D_SG > Incompatible inputs." )
        endif
        !
    ...
    function getAxis_cVector3D_SG( self, comp_lbl ) result( comp )
        ...
        !
        if( ( .NOT. self%is_allocated ) ) then
            call errStop( "getAxis_cVector3D_SG > self not allocated." )
        endif
        !
    ...
    subroutine read_cVector3D_SG( self, funit, ftype )
        ...
        if( ok ) then
            ...
            if((index(isbinary, "yes") > 0 .OR.index(isbinary, "YES") > 0) &
                  .AND.   .NOT. binary ) then
                call errStop( "read_cVector3D_SG > Unable to read from unformatted file ["//trim(fname)//"]." )
            elseif((index(isbinary, "no") > 0 .OR.index(isbinary, "NO") > 0) &
                  .AND.binary) then
                call errStop( "read_cVector3D_SG > Unable to read from formatted file ["//trim(fname)//"]." )
            endif
            !
            read(funit) Nx, Ny, Nz, grid_type
            !
            if(  .NOT. self%is_allocated ) then
                call errStop( "read_cVector3D_SG > self not allocated." )
            elseif( self%grid_type .NE. grid_type ) then
                call errStop( "read_cVector3D_SG > Incompatible grid_type." )
            elseif( ( self%nx .NE. Nx ).OR. &
                    ( self%ny .NE. Ny ).OR.( self%nz .NE. Nz ) ) then
                call errStop( "read_cVector3D_SG > Wrong size on input from ["//trim(fname)//"]." )
            endif
            !
            read(funit) self%x
            read(funit) self%y
            ...
            call errStop( "read_cVector3D_SG: unable to open file" )
        endif
        !
    ...
    subroutine write_cVector3D_SG( self, funit, ftype )
        ...
        if( ok ) then
            ...
            if( ( index( isbinary, "yes" ) > 0 .OR. index(isbinary, "YES") > 0 ) &
                  .AND.   .NOT. binary) then
                call errStop( "write_cVector3D_SG > Unable to write to unformatted file ["//trim(fname)//"]." )
            elseif( ( index( isbinary,"no" ) > 0 .OR. index( isbinary, "NO" ) > 0 ) &
                  .AND.binary) then
