...
contains
    ...
    function cVector3D_MR_ctor( grid, grid_type ) result(  self )
        ...
        if( .NOT. self%is_allocated ) then
            call errStop( "cVector3D_MR_ctor > Unable to allocate vector." )
        endif
        !
    ...
    !
    subroutine toSG_cVector3D_MR( self, vector_sg )
        implicit none
        !
        class( cVector3D_MR_t ), intent( in ) :: self
            ...
            case( EDGE )
                ...
                select type( grid => self%grid )
                    ...
                    class is( Grid3D_MR_t )
                        ...
                        !
                    class default
                        call errStop( "toSG_cVector3D_MR > Unclassified grid" )
                    !
                end select
                !
            case default
                call errStop( "toSG_cVector3D_MR > Unrecognized grid type." )
            !
        end select
        !
    end subroutine toSG_cVector3D_MR
    !
    !> No subroutine briefing
    !
    ...
    function dotProd_cVector3D_MR( self, rhs ) result( cvalue )
        ...
        !
        if( .NOT. self%is_allocated ) then
            call errStop( "dotProd_cVector3D_MR > self not allocated." )
        endif
        !
        cvalue = C_ZERO
        !
    ...
    subroutine getReal_cVector3D_MR( self, r_vector )
        ...
        select type ( r_vector )
            ...
            class is( rVector3D_MR_t )
                !
                do i = 1, size( r_vector%sub_vector )
                    !
                    r_vector%sub_vector(i)%x = real( self%sub_vector(i)%x, kind=prec )
                    r_vector%sub_vector(i)%y = real( self%sub_vector(i)%y, kind=prec )
                    r_vector%sub_vector(i)%z = real( self%sub_vector(i)%z, kind=prec )
                    !
                enddo
                !
            class default
                call errStop( "getReal_cVector3D_MR > Undefined r_vector" )
                !
        end select
        !
