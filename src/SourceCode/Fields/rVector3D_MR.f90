...
contains
    ...
    !> No subroutine briefing
    !
    subroutine SGtoMR_rVector3D_MR( target_sg, target_mr )
        implicit none
        !
        type( rVector3D_SG_t ), intent( in ) :: target_sg
        ...
        if( .NOT. target_sg%is_allocated ) then
            call errStop( "SGtoMR_rVector3D_MR > target_sg not allocated" )
        endif
        !
        if( .NOT. target_mr%is_allocated ) then
            call errStop( "SGtoMR_rVector3D_MR > target_mr not allocated" )
        endif
        !
        !>
        write( *, * ) "SGtoMR_rVector3D_MR SG: ", target_sg%grid%nx, target_sg%grid%ny, target_sg%grid%nz, target_sg%grid%n_grids
        ...
        !
        sg_vector_l = rVector3D_SG_t( target_sg%grid, EDGE )
        !
        call sg_vector_l%setArray( cmplx( array_l, 0.0, kind=prec ) )
        !
        sg_vector_el = rVector3D_SG_t( target_sg%grid, EDGE )
        !
        ...
        select type( target_mr )
            ...
            class is( rVector3D_MR_t )
                ...
                !
            class default
                call errStop( "SGtoMR_rVector3D_MR > Unclassified target_mr" )
            !
        end select
        !
