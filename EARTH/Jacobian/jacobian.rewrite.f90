  ...
  subroutine lFmult_cvector(E)
     ...
     do k=1,nz
       do j=2,ny
          ! Zero longitude
          i=1
          call leng_xijk2(nx,i,j,k,x,y,z,xlen)
          lF%x(i,j,k)=xlen
          do i=2,nx
...
! * divides E on FACES by corresponding area elements
  ...
  ! *
  ! * July 2012 modification: first compute resistivity on faces,
  ! * then compute final vector l_F * rho_F * S_F^(-1).
  ! * This is temporary. Will decompose into several routines in
  ...
  subroutine operatorL(resist,vecF,grid)
	...
	call create_rvector(grid,resistF,FACE)
    ...
    do k=1,nz
      ...
      do j=1,ny
		! loop over longitudes including i=nx (nx+1'th fields same as i=1)
        do i=1,nx
		  call leng_xijk2p(i,j,k,x,y,z,lp)
		  if (i==1) then
			call leng_xijk2(nx,i,j,k,x,y,z,l)
			call leng_xijk2m(nx,j,k,x,y,z,lm)
		  else
            call leng_xijk2(i-1,i,j,k,x,y,z,l)
			call leng_xijk2m(i-1,j,k,x,y,z,lm)
		  end if

          call area_sijk(j,k,y,z,S)

		  if (i==1) then
			resistF%x(i,j,k)=(lp*resist%v(i,j,k)+lm*resist%v(nx,j,k))/l
		  else
			resistF%x(i,j,k)=(lp*resist%v(i,j,k)+lm*resist%v(i-1,j,k))/l
		  end if
		  vecF%x(i,j,k)= l*resistF%x(i,j,k)/S

		  call leng_yijk2p(j,k,y,z,lp)
		  if (j==1) then
		  else
            call leng_yijk2(j-1,k,y,z,l)
			call leng_yijk2m(j-1,k,y,z,lm)
		  end if
          call area_sjki(i,j,k,x,y,z,S)

		  if (j==1) then
			resistF%y(i,j,k)=R_ZERO  !undefined
		  else
			resistF%y(i,j,k)=(lp*resist%v(i,j,k)+lm*resist%v(i,j-1,k))/l
		  end if
		  vecF%y(i,j,k)= l*resistF%x(i,j,k)/S

		  call leng_zijk2p(k,z,lp)
		  if (k==1) then
			lm=lp
		  else
		    call leng_zijk2(k-1,z,l)
			call leng_zijk2m(k-1,z,lm)
		  end if
          call area_skij(i,j,k,x,y,z,S)

		  if (k==1) then
			vecF%z(i,j,k)=(lp*resist%v(i,j,k))/l
		  else
			vecF%z(i,j,k)=(lp*resist%v(i,j,k)+lm*resist%v(i,j,k-1))/l
		  end if
		  vecF%z(i,j,k)= l*resistF%x(i,j,k)/S
    ...
    ! NEED TO RETHINK THESE BOUNDARY FACES VERY CAREFULLY. ONLY NEED THEM TO COMPUTE
    ! THE ADJOINT CURL ON BOUNDARY EDGES FOR SENSITIVITIES WITH RESPECT TO B.C. RIGHT?
	call leng_zijk2m(nz,z,lm)
    do j=1,ny
      do i=1,nx
  ...
  subroutine operatorLt(resist,vecF,grid)
    ...
    do k=1,nz
      ...
      do j=1,ny
		! loop over longitudes including i=nx (nx+1'th fields same as i=1)
        do i=1,nx
		  call leng_xijk2(i,i+1,j,k,x,y,z,l)
          call area_sijk(j,k,y,z,S)
		  Sp=S
