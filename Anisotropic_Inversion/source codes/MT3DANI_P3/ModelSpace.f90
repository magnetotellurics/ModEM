...
!    QtoModelParam, sigC
  ...
  type :: modelParam_t
     ...
     type(rscalar)		:: cellCond(3) !变为三维数组，存放三个主轴电导率
...
include "ModelMap.inc" !Linux系统下，在include前加上#号就没有问题了，windows加#号编译不了
...
!
   ...
   subroutine create_modelParam(grid,paramtype,m,v,vAir)
     ...
     type (rscalar), intent(in), optional   :: v(3)
     ...
     do ia = 1,3  !修改为数组形式
       call create_rscalar(grid,m%cellCond(ia),CELL_EARTH)
     enddo
     m%Nx = grid%Nx
     m%Ny = grid%Ny
  ...
  !************************************************************
   subroutine deall_modelParam(m)
     ...
     if(m%allocated) then
        do ia = 1,3
          call deall_rscalar(m%cellCond(ia))
        enddo
        nullify(m%grid)
        m%allocated = .false.
        m%zeroValued = .false.
...
!**********************************************************************
   ...
   !    here implemented for 3D numerical grid
     ...
     if(.not. m1%zeroValued .and. .not. m2%zeroValued) then
        do ia = 1,3
          r0(ia) = dotProd_rscalar_f(m1%cellCond(ia), m2%cellCond(ia))
          r = r + r0(ia)
        enddo       
     endif
...
!**********************************************************************
   ...
   subroutine zero_modelParam(m)
     ...
     do ia = 1,3
       call zero_rscalar(m%cellCond(ia))
     enddo
...
!**********************************************************************
   ...
   subroutine linComb_modelParam(a1,m1,a2,m2,m)
     ...
     do ia = 1,3
       m%cellCond(ia)%v = a1*m1%cellCond(ia)%v + a2*m2%cellCond(ia)%v
     enddo
  ...
  !  computes mOut = a * mIn + mOut for modelParam object m and real scalar a
   ...
   subroutine copy_modelParam(mOut,mIn)
     ...
     do ia = 1,3
       mOut%cellCond(ia)%v = mIn%cellCond(ia)%v
     enddo
  ...
  subroutine getValueUpdated_modelParam(m,updated)
    ...
    ndim_lbfgs = 3 * m0_lbfgs%Nx * m0_lbfgs%Ny * m0_lbfgs%NzEarth !少一维
      ...
      do ix=1,m_in%Nx
         do iy=1,m_in%Ny
            do iz=1,m_in%NzEarth
                m_out(cont) = m_in%cellCond(ia)%v(ix,iy,iz)
                cont = cont + 1
      ...
      do ix=1,m_out%Nx
         do iy=1,m_out%Ny
            do iz=1,m_out%NzEarth
                m_out%cellCond(ia)%v(ix,iy,iz) = m_in(cont)
                cont = cont + 1
            enddo
         enddo
      enddo
    ...
    subroutine cal_anisCsNorm(m,aniNorm,lambdaANI)
      ...
      call anisCsNorm(nx,ny,nzEarth,lambdaANI,m%cellCond(1)%v,m%cellCond(2)%v,m%cellCond(3)%v,aniNorm)
    ...
    subroutine cal_anisCsVec(m,aniVec,lambdaANI)
      ...
      call anisCsVec(nx,ny,nzEarth,lambdaANI,m%cellCond(1)%v,m%cellCond(2)%v,m%cellCond(3)%v, &
              aniVec%cellCond(1)%v,aniVec%cellCond(2)%v,aniVec%cellCond(3)%v)
