   ...
   nTx = d1%nTx
     ...
     ! First, distribute the current model to all workers
      ...
      !end if
        ...
        call UpdateRho(sigma) ! KONG, 2018-10-21 
...
!         end do
       ...
       job_name= 'PQmultT'
       if(returne_m_vectors) then
         call Master_job_Distribute_Taskes(job_name,nTx,sigma,eAll_out,eAll_temp,dsigma,s_hat)
       else
         call Master_job_Distribute_Taskes(job_name,nTx,sigma,eAll_out,eAll_temp,dsigma)
       endif
                endtime=MPI_Wtime()
                time_used = endtime-starttime
        !DONE: Received soln for all transmitter from all nodes
...
Subroutine Master_job_Distribute_Model(sigma,delSigma)
 ...
 if (send_delSigma) then
        ...
        do dest=1,number_of_workers
         ...
         call create_model_param_place_holder(sigma) !modified for anisotropy
         call pack_model_para_values(sigma) !modified for anisotropy
         call MPI_BCAST(sigma_para_vec,Nbytes, MPI_PACKED,0, MPI_COMM_WORLD,ierr)
end if
...
Subroutine Worker_job(sigma,d)
   ...
   type(modelParam_t) :: dsigma_temp, Qcomb ! added by Kong, 2018-11-22 
...
if (trim(worker_job_task%what_to_do) .eq. 'FORWARD') then
          ...
          worker_job_task%taskid=taskid
		       ...
		       call deall_solver() ! KONG, 2018-10-20
...
!                  call deall_solver() ! KONG, 2018-10-20
                  ...
                  do ipol=1,nPol_MPI
		            ...
		            call MPI_RECV(e_para_vec, Nbytes, MPI_PACKED, 0, FROM_MASTER,MPI_COMM_WORLD, STATUS, ierr)
                    call Unpack_e_para_vec(e)
                  end do
                  write(6,'(a12,a18,i5,a12)') node_info, ' Finished Receiving ' , orginal_nPol, ' from Master'
          
         ! debug, added by Kong Wenxin
         dsigma_temp = sigma
         call zero(dsigma_temp)
         Qcomb = sigma
         call zero(Qcomb)
...
subroutine Master_job_Distribute_Taskes(job_name,nTx,sigma,eAll_out,eAll_in,dsigma,s_hat)
     implicit none
        ...
        type(modelParam_t)           :: dsigma_temp  !added by Kong, 2018-11-22
                          ...
                          do ipol=1,nPol_MPI
	            			  ...
	            			  call MPI_SEND(worker_job_package,Nbytes, MPI_PACKED,dest, FROM_MASTER, MPI_COMM_WORLD, ierr)
	            			     ...
	            			     if (trim(job_name).eq. 'JmultT' .or. trim(job_name).eq. 'Jmult' &
	            			         .or. trim(job_name).eq. 'PQmultT')then
								 ...
								 ! In the 2D MT case there are one polarisation.
										...
										if (trim(job_name).eq. 'PQmultT') then 
							               do ipol1=1,nPol_MPI
				    			               ...
				    			               call Pack_e_para_vec(eAll_out%solns(which_per))
				    			               call MPI_SEND(e_para_vec, Nbytes, MPI_PACKED, dest,FROM_MASTER, MPI_COMM_WORLD, ierr) 
							               end do
                                        endif   
                                 end if	  
...
10         continue
      ...
      !answers_to_receive = nTx*nPol_MPI
        ...
        do while (received_answers .lt. answers_to_receive)
            ...
            if(trim(job_name).eq. 'PQmultT') then 
                call create_modelParam(grid,LOGE,dsigma_temp)
                call create_model_param_place_holder(dsigma_temp)
                call MPI_RECV(sigma_para_vec,Nbytes, MPI_PACKED, who, FROM_WORKER,MPI_COMM_WORLD, STATUS, ierr)
                call Unpack_model_para_values(dsigma_temp)                
                call linComb_modelParam(ONE,dsigma,ONE,dsigma_temp,dsigma)
                if(present(s_hat)) s_hat(which_per) = dsigma_temp   
            else      
                call create_e_param_place_holder(eAll_out%solns(which_per))
...
if(trim(job_name).eq. 'PQmultT') then 
   dsigma_temp = dsigma  
   call scMult_modelParam(ONE/2,dsigma_temp,dsigma)
   call deall_modelParam(dsigma_temp) !added by Kong, 2018-11-22 
endif
