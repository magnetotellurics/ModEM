...
Contains
  ...
  subroutine Jrows(iTx,iDt,iRx,sigma0,emsoln,Jreal,Jimag)
   ...
   call deall_solver() !release memory
  ...
  subroutine JmultT_TX(sigma0,d,dsigma,emsoln)
   ...
   real(kind=prec) :: period !debug
   character*80 :: combfile !debug
...
!    write(combfile,'(f10.3)') period     
!    open(1024,file='pseudo forward solution of per='//trim(adjustl(combfile))//' for ipol=1.dat')
!    call writeEsol(e%pol(1),1024)
!    close(1024)
!    open(1024,file='pseudo forward solution of per='//trim(adjustl(combfile))//' for ipol=2.dat')
!    call writeEsol(e%pol(2),1024)
!    close(1024)    
!    !pause
!    !end of debug   
!
...
!    period = txDict(iTx)%period 
!    write(combfile,'(f10.3)') period     
!    open(1024,file='forward solution of per='//trim(adjustl(combfile))//' for ipol=1.dat')
!    call writeEsol(e0%pol(1),1024)
!    close(1024)
!    open(1024,file='forward solution of per='//trim(adjustl(combfile))//' for ipol=2.dat')
!    call writeEsol(e0%pol(2),1024)
!    close(1024)    
!    !pause
!    !end of debug 
  ...
  subroutine JmultT(sigma0,d,dsigma,eAll,JT_multi_Tx_vec)
   ...
   if(savedSolns) then
      ...
      !call write_solnVectorMTX(eAll,'invfield.txt') !debug
  ...
  subroutine fwdPred(sigma,d,eAll)
   ...
   !call write_solnVectorMTX(eAll,'fwdfield.txt') !debug
