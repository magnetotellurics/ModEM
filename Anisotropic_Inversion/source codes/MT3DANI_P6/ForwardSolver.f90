...
Contains
   ...
   !**********************************************************************
   subroutine initSolver(iTx,rho,grid,e0,e,comb)
   ...
   type(modelParam_t),intent(in), target		:: rho
   ...
   call UpdateFreqRho(txDict(iTx)%omega,rho) ! update frequency and resitivity
...
!   call deall_RHS(b0)
   ...
   do iMode = 1,e0%nPol
      ...
      call Mackie_solve(b0,omega,e0%pol(imode),imode)
   enddo
   ...
   do iMode = 1,e%nPol
      ...
      call Mackie_solve(comb%b(e%Pol_index(iMode)),omega,e%pol(imode),imode)
   enddo
   ...
   !*****************************************
   subroutine deall_solver()
   
     call deall_RHS(b0)
     call modelOperatorCleanup()
     
   end subroutine deall_solver
