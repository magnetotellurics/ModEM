...
module sg_scalar
  ...
  public		::   create_rscalar,  create_cscalar, create_iscalar, &
       ...
       subtractr_rcscalar, subtracti_rcscalar, dMult_rscalar, zdotu_rscalar_f
...
!****************************************************************** Lana
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!KONG!!!!!!!!!!!!!!!!!!!!!!!!  
    subroutine subtractr_rcscalar(E1, E2, E3)
    ...
    if(.not.E3%allocated) then
       ...
       if((E1%nx == E2%nx).and.(E1%ny == E2%ny).and.(E1%nz == E2%nz).and.&
          ...
          if ((E1%gridType == E2%gridType).and.(E1%gridType == E3%gridType)) then
             ...
             ! subtract v-component
!             E3%v = E1%v - dreal(E2%v)
             
             ! computation done only for internal nodes
             do ix = 2, E1%nx
               do iy = 2, E1%ny
                 do iz = 2, E1%nz
                   E3%v(ix,iy,iz)=E1%v(ix,iy,iz)-dreal(E2%v(ix,iy,iz))
                 enddo
               enddo
             enddo
  ...
  end subroutine subtractr_rcscalar


      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!KONG!!!!!!!!!!!!!!!!!!!!!!!!  
    subroutine subtracti_rcscalar(E1, E2, E3)
...
!             E3%v = E1%v - dimag(E2%v)
            ...
            do k=2,n
	            do j=2,m
		            do i=2,l
                  E3%v(i,j,k)=E1%v(i,j,k) - dimag(E2%v(i,j,k))
		            end do
		          end do
	          end do
  ...
  end subroutine subtracti_rcscalar


  subroutine dMult_rscalar(E1,E2,E3)
    implicit none
    type (rscalar), intent(in)       :: E1, E2
    type (rscalar), intent(inout)	 :: E3
	  integer						     :: i,j,k,l,m,n
    ...
    if((.not.E1%allocated).or.(.not.E2%allocated)) then
       write(0,*) 'RHS not allocated yet for zdotu_rscalar_f'
       stop
	  end if

    ! check to see if LHS (E3) is active (allocated)
    if(.not.E3%allocated) then
       write(0,*) 'LHS was not allocated for dMult_rscalar'
       stop
    else
...
!             E3%v = E1%v * E2%v
            ...
            do k=2,n
	            do j=2,m
		            do i=2,l
                  E3%v(i,j,k)=E1%v(i,j,k)*E2%v(i,j,k)
		            end do
		          end do
	          end do

          else
             write (0, *) 'not compatible usage for dMult_rscalar'
             stop
          end if

       else

          write(0, *) 'Error:dMult_rscalar: scalars not same size'
          stop

       end if
    end if

  end subroutine dMult_rscalar  
  
  
  !***************************************
  function zdotu_rscalar_f(E1,E2) result(c)
    implicit none
    type (rscalar), intent(in)       :: E1, E2
    real(kind=prec)		             :: c
	integer						     :: i,j,k,l,m,n
    ...
    if((.not.E1%allocated).or.(.not.E2%allocated)) then
       ...
       do k=2,n
	     do j=2,m
		   do i=2,l
             c=c+E1%v(i,j,k)*E2%v(i,j,k)
		   end do
		 end do
	   end do
	end if

  end function zdotu_rscalar_f
