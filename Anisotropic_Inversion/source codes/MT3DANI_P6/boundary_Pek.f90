...
Contains
  ...
  subroutine BC_x0_Pek(imode,period,grid3D,Cond3D,E0,BC)
    ...
    type(rscalar), intent(in)	:: Cond3D(6)
...
!	   write(1010,*) grid2D%Dz(iz)  ! debug
    ...
    call slice(nl,nh,nv,Cond3D,1,'y',Cond2D)
!    !debug
!    do ia = 1,6
!      iof = 1088 + ia
...
!    !debug    
    ...
    call copy_h2cb(nl1,nh1,nv1,hx2d,hy2d,hz2d,BC,'x','Min')							 
		deallocate(hx2d,hy2d,hz2d)	
		do ia = 1,6
      call deall_rscalar_2d(Cond2D(ia))
		enddo
    
		call slice(nl,nh,nv,Cond3D,nl,'y',Cond2D)
		allocate(hx2d(ln),hy2d(ln),hz2d(ln))
		hx2d=C_ZERO
		hy2d=C_ZERO
		hz2d=C_ZERO
    call d2atbp(np2_2d,np3_2d,nh1,nv1,nzap1,grid2D%Dy,  &
         grid2D%Dz,nv,period,imode,Cond2D,hx2d,hy2d,hz2d,'x') 
    call copy_h2cb(nl1,nh1,nv1,hx2d,hy2d,hz2d,BC,'x','Max')	
		deallocate(hx2d,hy2d,hz2d)
    do ia = 1,6
      call deall_rscalar_2d(Cond2D(ia))
    enddo
  ...
  subroutine BC_y0_Pek(imode,period,grid3D,Cond3D,E0,BC)
    ...
    call slice(nh,nm,nv,Cond3D,1,'x',Cond2D) ! direction
    allocate(hx2d(ln),hy2d(ln),hz2d(ln))
		hx2d=C_ZERO
		hy2d=C_ZERO
		hz2d=C_ZERO
    ...
    call copy_h2cb(nh1,nm1,nv1,hx2d,hy2d,hz2d,BC,'y','Min') ! strike
		deallocate(hx2d,hy2d,hz2d)
		do ia = 1,6
      call deall_rscalar_2d(Cond2D(ia))
		enddo
    
		call slice(nh,nm,nv,Cond3D,nm,'x',Cond2D)
    allocate(hx2d(ln),hy2d(ln),hz2d(ln))
		hx2d=C_ZERO
		hy2d=C_ZERO
		hz2d=C_ZERO
    call d2atbp(np2_2d,np3_2d,nh1,nv1,nzap1,grid2D%Dy,  &
         grid2D%Dz,nv,period,imode,Cond2D,hx2d,hy2d,hz2d,'y')     
    call copy_h2cb(nh1,nm1,nv1,hx2d,hy2d,hz2d,BC,'y','Max')
		deallocate(hx2d,hy2d,hz2d)
    do ia = 1,6
      call deall_rscalar_2d(Cond2D(ia))
    enddo
