...
Contains
  ...
  subroutine boufld(m,n,nlayr,nb,y,z,ihpol,period,cond,ex,ey,hx,hy,hz)
    ...
    type(rscalar_2d),intent(in)   :: cond(6)
...
!   in the earth
    ...
    do j=2,m-1
      dire=sy(j)/sy(m)
      ! top boundary
      ex(1,j)=ex(1,1)+dire*(ex(1,m)-ex(1,1))
      ey(1,j)=ey(1,1)+dire*(ey(1,m)-ey(1,1))
			do k=1,nb  ! in the air
        hx(k,j)=hx(k,1)+dire*(hx(k,m)-hx(k,1))
        hy(k,j)=hy(k,1)+dire*(hy(k,m)-hy(k,1))
			end do
      hz(1,j)=hz(1,1)+dire*(hz(1,m)-hz(1,1))
      ! bottom boundary
      ex(n,j)=ex(n,1)+dire*(ex(n,m)-ex(n,1))
      ey(n,j)=ey(n,1)+dire*(ey(n,m)-ey(n,1))
      hx(n,j)=hx(n,1)+dire*(hx(n,m)-hx(n,1))
      hy(n,j)=hy(n,1)+dire*(hy(n,m)-hy(n,1))
      hz(n,j)=hz(n,1)+dire*(hz(n,m)-hz(n,1))
    end do
