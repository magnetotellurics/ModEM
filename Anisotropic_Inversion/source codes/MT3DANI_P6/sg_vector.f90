...
!  END INTERFACE
  ...
  public		::   create_rvector,  create_cvector, &
       ...
       cmplx_rvector_f, real_cvector_f, imag_cvector_f, zdotu_cvector_f, writeEsol
...
Contains
  ...
  !***************************************
  function zdotu_cvector_f(E1,E2) result(c)
    implicit none
    type (cvector), intent(in)       :: E1, E2
    complex(kind=prec)		         :: c
	integer						     :: i,j,k,l,m,n
    ...
    if((.not.E1%allocated).or.(.not.E2%allocated)) then
       write(0,*) 'RHS not allocated yet for zdotu_cvector_f'
       return
	else
	   c = C_ZERO
       l=E1%Nx
       ...
       do k=1,n-1
	     do j=2,m
		   do i=2,l
             c=c+E1%z(i,j,k)*E2%z(i,j,k)
		   end do
		 end do
	   end do
    endif

  end function zdotu_cvector_f
  
  !**************************
  subroutine writeEsol(E,iof)
    implicit none
	integer iof
    type (cvector), intent(in)            :: E
	integer i,j,k,l,m,n
    ...
    if(.not.E%allocated) then
       write(0,*) 'input not allocated yet for write_cvector'
    else
       l=E%Nx
	   m=E%Ny
	   n=E%Nz+1
       do i=1,l
	     do k=2,n
		   do j=2,m
             write(iof,*) i,j,k,real(E%x(i,j,k)),imag(E%x(i,j,k))
		   end do
		 end do
	   end do
       do j=1,m
	     do k=2,n
		   do i=2,l
             write(iof,*) i,j,k,real(E%y(i,j,k)),imag(E%y(i,j,k))
		   end do
		 end do
	   end do
       do k=1,n-1
	     do j=2,m
		   do i=2,l
             write(iof,*) i,j,k,real(E%z(i,j,k)),imag(E%z(i,j,k))
		   end do
		 end do
	   end do
  ...
  end subroutine writeEsol


  !**************************
  subroutine readEsol(E,iof)
    implicit none
	  integer iof
    type (cvector), intent(inout)            :: E
	  integer i,j,k,ix,jy,kz,l,m,n
	  real(kind=prec) :: a,b
    ...
    if(.not.E%allocated) then
       ...
       do i=1,l
	     do k=2,n
		   do j=2,m
             read(iof,*) ix,jy,kz,a,b
             E%x(i,j,k) = dcmplx(a,b)
		   end do
		 end do
	   end do
       do j=1,m
  ...
  end subroutine readEsol


  !**************************
  subroutine writeAllEsol(E,iof)
    implicit none
	integer iof
    type (cvector), intent(in)            :: E
	integer i,j,k,l,m,n
