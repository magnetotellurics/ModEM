...
! Version 7.0  October 20, 2009.    Rearranged the code to encapsultate subroutines into a Fortran module.
...
! Version 4.0  June 14, 2007.       N-layer version for aribitrary dipole orientation.
...
!
! Invesion sensitivity computations:
!
...
!--------------------------------------------------------
    ...
    private :: allocate_dipole1d, initialize_dipole1d, deallocate_dipole1d, setupsite
    ...
    private :: hed_coef_nlayer, hed_pot_nlayer, ved_coef_nlayer, ved_pot_nlayer, PreComputePotCoeffs
...
!
! This is the main subroutine that you need to call after initializine all
...
!
! Compute derivatives with respect to conductivitity:
!
        if (linversion) then 
...
!
    ...
    do i = 2,iTxlayer
      ...
      rjexp  = Rm(i-1)*expmgh(i-1)   ! then we apply attentuation to bottom of layer
      ...
      Sm(i)  =  (sgmosgp + sjexp) / ( 1.d0 + sgmosgp*sjexp)     
        ...
        if (linversion) then
            ...
            ! Lastly, apply chain rule for layers 1:i-2 to propagate deriv for layers already intialized on previous iterations:
   ...
   ! Derivatives for b terms:           
    ...
    ! Derivatives for d terms:          
     ...
     ! dAzdsig:
        ...
        ! add on terms for deriviative in iRxlayer.  Note that a,b,c,d derivatives have this already, but gg term doesn't
    ...
    ! dAzdsigdz:    
        ...
        ! add on terms for deriviative in iRxlayer.  Note that a,b,c,d derivatives have this already, but gg terms don't
...
!
! Layer up/down attentuation terms:
!
    expcoef1 = exp(-gg*heightRx)  ! upgoing term 
    expcoef2 = exp(-gg*depthRx)  ! downgoing term
!
...
!
! Subroutine to precompute potential coeffcients across lambda range so that many sites can be
! computed rapidly by spline iterpolation in lambda of the precomputed coefficients.
!
! Kerry Key
! Scripps Institution of Oceanography
!
! Version 1.2       November 5, 2009        Fixed a bug J. Gunning spotted in arry index for 2nd order interpolation coeffs.
...
!
    ...
    allocate( ilayersInterp(nlayersInterp) )  ! this small array is used to extract the required layers for intperpolation
    icnt = 0
    do i = 1,nlay1D
        if ( iRxLayerInterp(i) /= 0) then
            icnt = icnt  + 1
...
!
! Now test drive through every decade of lambda range and identify where potential coeffcients are of reasonable size:
!
     pot_coef_tol = 1d-30
...
!
    if (lhed)  then
        ...
        !
        ! KWK Note: These takeup most of the CPU time for each call to Dipole1D with linversion = true.
