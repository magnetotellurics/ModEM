...
! (S1) and field1d_s2.f90 (S2) on the identical
...
! eq (5)-(6) -- see the NOTE above sourceField1d in field1d.f90
! -- after which field1d.f90 and field1d_s2.f90 agree exactly (up
! to a real normalization constant) in testing/test_s1_vs_s2_l1m0.f90,
! and field1d_s2.f90 was already independently confirmed to match
...
!   Mode1 (l=1,m=0, zonal): coefl(m=0)=1, coefl(m=+-1)=0 -- Zyx = Ey/Hx.
!   Mode2 (l=1,m=+-1): coefl(m=+1)=-0.5, coefl(m=-1)=+0.5 -- Zxy = Ex/Hy,
!     evaluated at phi=90deg (Ex,Hy are ~sin(phi), zero at phi=0; the exact
!     phi chosen doesn't affect the ratio -- verified analytically).
!
! Units: field1d.f90/field1d_s2.f90 output raw SI (E in V/m, H in
...
! ****************************************************************************
    ...
    ! ---- hand-built grid: phi mid-cell at 90deg (Mode2's Zxy needs this --
    ...
    i_nodeph = 1   ! node(i=1)     = 45deg  (H%y, E%x -- "Yt-based", mid-theta; phi-independent for Mode1)
    ...
    coeff(2) = dcmplx(1.0d0, 0.0d0)   ! l=1,m=0 -- matches MTsource.1000sec.Mode1.prm

    write(*,*) '--- Mode1 (zonal, l=1 m=0): Zyx = Ey/Hx ---'
    ...
    coeff = dcmplx(0.0d0,0.0d0)
    coeff(3) = dcmplx(-0.5d0, 0.0d0)  ! l=1,m=+1 -- matches MTsource.1000sec.Mode2.prm
    coeff(4) = dcmplx( 0.5d0, 0.0d0)  ! l=1,m=-1

    write(*,*) '--- Mode2 (l=1 m=+-1): Zxy = Ex/Hy, evaluated at phi=90deg ---'
