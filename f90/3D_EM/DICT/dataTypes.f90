...
module dataTypes
  ...
  ! These are data type for general CSEM problem
  ! Added by Oat on Mar. 03, 2011
  ...
  ! Add DC-geoelectric data type. Naser Meqbel Dec. 11 2012
  integer, parameter   :: Pole_Pole_DC_Rho = 12
...
! Initializes and sets up data type dictionary
  subroutine setup_typeDict()
     ...
     allocate(typeDict(12),STAT=istat)
     ...
     typeDict(Pole_Pole_DC_Rho)%name = 'Pole_Pole_DC_Rho'
     typeDict(Pole_Pole_DC_Rho)%isComplex = .false.
     typeDict(Pole_Pole_DC_Rho)%tfType    = Pole_Pole_DC_Rho
     typeDict(Pole_Pole_DC_Rho)%units = '[]'
     typeDict(Pole_Pole_DC_Rho)%nComp     = 1
     allocate(typeDict(Pole_Pole_DC_Rho)%id(1),STAT=istat)
     typeDict(Pole_Pole_DC_Rho)%id(1)    = 'Pole_Pole_DC_Rho' 
  ...
  function ImpHeader(dataType) result (header)
    ...
    select case (dataType)
       ...
       case(Ex_Field, Ey_Field, Bx_Field, By_Field, Bz_Field)
          header = '# Tx_Dipole Tx_Period(s) Tx_Moment(Am) Tx_Azi Tx_Dip Tx_X(m) Tx_Y(m) Tx_Z(m) Code X(m) Y(m) Z(m) Component Real Imag Error'
       case(Pole_Pole_DC_Rho)
