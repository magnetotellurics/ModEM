...
!******************************************************************************
  subroutine dataResp(ef,Sigma,iDT,iRX,Resp,Binv,Eout,Bout)
  ...
  complex(kind=prec), intent(out), optional	:: Binv(2,2),Eout(2,2),Bout(3,2)
  ...
  complex(kind=prec)	:: det,i_omega,ctemp,Ex_res,Ey_res,Bx_res,By_res
  type(sparsevecC)		:: Lex,Ley,Lez,Lbx,Lby,Lbz,Lrx,Lry
  logical			:: ComputeHz,ComputeE
  COMPLEX    VALUE, X1, Y1
  INTEGER    NOUT
  real(kind=prec) :: HxAngle,ExAngle,HxAngle_ref,HyAngle,EyAngle
 ...
 selectcase (iDT)
	  ...
	  case (Bx_Field)
		   ...
		   Z(1) = dotProd_noConj_scvector_f(Lbx,ef%pol(1))
	  case (By_Field)
		   x = rxDict(iRX)%x
		   xyz = 2
		   call BfromESetUp(ef%grid,omega,x,xyz,Lby)		
		   Z(1) = dotProd_noConj_scvector_f(Lby,ef%pol(1))
	  case (Bz_Field)
		   x = rxDict(iRX)%x
		   xyz = 3
		   call BfromESetUp(ef%grid,omega,x,xyz,Lbz)   
		   Z(1) = dotProd_noConj_scvector_f(Lbz,ef%pol(1))	 
       case (Bz_Ampli_Phase)
		   x = rxDict(iRX)%x
		   xyz = 3
		   call BfromESetUp(ef%grid,omega,x,xyz,Lbz)   
		   tempZ(1) = dotProd_noConj_scvector_f(Lbz,ef%pol(1))	
       ...
       case (Bxy_Field)
		    ...
		    Bx_res=dotProd_noConj_scvector_f(Lbx,ef%pol(1))
		    By_res=dotProd_noConj_scvector_f(Lby,ef%pol(1))
			
     	    Z(1) = (cos(D2R*Rx_Azi)*Bx_res)+(sin(D2R*Rx_Azi)*By_res)	
       case (Bxy_Ampli_Phase)
            Rx_Azi=rxDict(iRX)%Rx_Azi
     	    x = rxDict(iRX)%x     	
		    xyz = 1
		    call BfromESetUp(ef%grid,omega,x,xyz,Lbx)			
		    ...
		    By_res=dotProd_noConj_scvector_f(Lby,ef%pol(1))
		    !write(*,*)"Rx_Azi: ",rxDict(iRX)%ID, Rx_Azi 
     	    tempZ(1) = (((cos(D2R*Rx_Azi))*Bx_res)+((sin(D2R*Rx_Azi))*By_res))
...
!****************************************************************************
  ...
  type(sparsevecC) :: Lex_rot,Ley_rot,Lbx_rot,Lby_rot,Lrx_rot,Lry_rot,Lex_rot_ampli,Lex_rot_phase,Lbx_rot_ampli,Lbx_rot_phase,Lbz_ampli,Lbz_phase
...
if (txDict(e0%tx)%tx_type =='CSEM' .or. txDict(e0%tx)%tx_type =='DC') then
  select case(iDT)  
    case (Exy_Ampli_Phase)
           ...
           !write(*,*)"Rx_Azi: Lrows",iRX, Rx_Azi	
		   c1= conjg(Z(1))/((abs(Z(1))**TWO)*dlog(10.0d0))
		   call scMult_sparsevecc (c1,Lex_rot,Lex_rot_ampli)
		   !write(*,*) "Ampli: ",c1,dreal(Z(1)),dimag(Z(1))
		   
		   !(180/pi) * i * conjg(E)/|E|^2 * Le
		   c1= R2D*dcmplx(0.0d0,1.0d0)*conjg(Z(1))/ABS(Z(1))**TWO
    ...
    case (Bxy_Ampli_Phase)
		   ...
		   Call dataResp(e0,Sigma0,Bxy_Field,iRX,Resp) !--> Z(1) is Bxy field
		   
		   !  Log10 (|e|)'=|e|' * 1/(|e|*log(10))=(e/|e|)* 1/(|e|*log(10))=  e/(|e|^2*log(10))
           !write(*,*)"Rx_Azi: Lrows",iRX, conjg(Z(1))	
		   c1= conjg(Z(1))/((abs(Z(1))**TWO)*dlog(10.0d0))
		   call scMult_sparsevecc (c1,Lbx_rot,Lbx_rot_ampli)
		   
		   !(180/pi) * i * conjg(B)/|B|^2 * Le
		   c1= dcmplx(0.0d0,1.0d0)*conjg(Z(1)) / (abs(Z(1))**TWO) *R2D
		   call scMult_sparsevecc (c1,Lbx_rot,Lbx_rot_phase)
		   
		   L(1)%L(1) = Lbx_rot_ampli 
		   L(2)%L(1) = Lbx_rot_phase
    ...
    case (Bz_Ampli_Phase)
        ...
        call BfromESetUp(e0%grid,omega,x,xyz,Lbz)
		   ...
		   Call dataResp(e0,Sigma0,Bz_Field,iRX,Resp) !--> Z(1) is Bz field
		   
		   !  Log10 (|e|)'=|e|' * 1/(|e|*log(10))=(e/|e|)* 1/(|e|*log(10))=  e/(|e|^2*log(10))
           !write(*,*)"Rx_Azi: Lrows",iRX, conjg(Z(1))	
		   c1= conjg(Z(1))/((abs(Z(1))**TWO)*dlog(10.0d0))
		   call scMult_sparsevecc (c1,Lbz,Lbz_ampli)
		   
		   !(180/pi) * i * conjg(E)/|E|^2 * Le
		   c1= dcmplx(0.0d0,1.0d0)*conjg(Z(1)) / (abs(Z(1))**TWO) *R2D
		   call scMult_sparsevecc (c1,Lbz,Lbz_phase)
  ...
  call deall_sparsevecc(Lbx_rot_ampli)
  call deall_sparsevecc(Lbx_rot_phase)
  call deall_sparsevecc(Lbz_ampli)
  call deall_sparsevecc(Lbz_phase) 
 return
end if
  ...
  select case(iDT)
     ...
     case(Full_Impedance_wAzimuth)
        ...
        do j = 1,2
           ...
           enddo
        enddo
        Call dataResp(e0,Sigma0,Full_Impedance_wAzimuth,iRX,Resp,Binv,Eout,Bout)
...
!if (typeDict(iDT)%tfType .eq. Full_Impedance .or. typeDict(iDT)%tfType .eq. Full_Interstation_TF .or. typeDict(iDT)%tfType .eq. Full_Vertical_Components .or. typeDict(iDT)%tfType .eq. Off_Diagonal_Impedance) then
  ...
  enddo
!end if 
if (typeDict(iDT)%tfType .eq. Full_Impedance_wAzimuth) then
 Rx_Azi= rxDict(iRX)%Rx_Azi
  cos2a = cos(2.0d0*Rx_Azi*D2R)
  sin2a = sin(2.0d0*Rx_Azi*D2R)
  cosa  = cos(Rx_Azi*D2R)
  sina  = sin(Rx_Azi*D2R)
...
!****************************************************************************
  ...
  selectcase (iDT)
      case(Full_Impedance, Phase_Tensor,Full_Impedance_wAzimuth)
