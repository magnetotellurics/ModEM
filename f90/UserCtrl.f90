...
module UserCtrl
  ...
  ! WERDT
  !
  character*1, parameter  	:: CONF_FILE = 'W'
  !
  logical             		:: config_file_opened, config_file_exists
  !
...
Contains
  ...
  subroutine parseArgs(program,ctrl)
     ...
     if(narg .gt. 0) then
		...
		!
		write(*,*) '[CONF_FILE]'
        write(*,*) ' -W  rFile_Config'
        write(*,*) '  Reads a configuration file'
        write(*,*) '[READ_WRITE]'
	 ...
	 if( job .eq. CONF_FILE ) then !W
        if ( narg .ne. 1 ) then
			write(0,*) 'Usage: -W  rFile_Config'
			stop
        else
			ctrl%job = job
			ctrl%rFile_Config = temp(1)
			deallocate(temp)
	    end if
	 else
		 select case (job)
