...
!ccyr       CALL MPI_PACK_SIZE(80*20, MPI_CHARACTER,        MPI_COMM_WORLD, Nbytes1,  ierr)
...
!ccyr        Nbytes=(Nbytes1+Nbytes2+Nbytes3)+1
 ...
 subroutine pack_userdef_control(ctrl)
    implicit none
       ...
       index=1
!       print *, "mpi_sub_pack ",ctrl%option,ctrl%prefix
!ccyr
...
!   print *, "mpi_unpack ",ctrl%option,ctrl%prefix
...
subroutine check_userdef_control_MPI (which_proc,ctrl)
	...
	character(20), intent(in)           :: which_proc
       ...
       write(6,*)trim(which_proc),' : ctrl%rstoreSolnsInfile ',ctrl%storeSolnsInfile
