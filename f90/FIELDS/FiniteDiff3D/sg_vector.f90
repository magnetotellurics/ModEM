...
Contains
  ...
  subroutine diagMult_crvector(E1, E2, E3)
    ...
    if(.not.E1%allocated) then
       write(0,*) 'E1 not allocated yet for diagMult_crvector'
       return
    endif
	
    if(.not.E2%allocated) then
       write(0,*) 'E2 not allocated yet for diagMult_crvector'
       return
    endif

    ! check to see if LHS (E3) is active (allocated)
    if(.not.E3%allocated) then
       write(0,*) 'E3 was not allocated for diagMult_crvector'
    else
  ...
  function imag_cvector_f(E1) result (E2)
    ...
    subroutine printRVector( E, io_unit, title )
        implicit none
        !
        class( rvector ), intent( in ) :: E
        ...
        if( E%gridType == EDGE ) then
            !
            NdX = (/E%nx, E%ny + 1, E%nz + 1/)
            NdY = (/E%nx + 1, E%ny, E%nz + 1/)
            NdZ = (/E%nx + 1, E%ny + 1, E%nz/)
            !
        else if( E%gridType == FACE ) then
            !
            NdX = (/E%nx + 1, E%ny, E%nz/)
            NdY = (/E%nx, E%ny + 1, E%nz/)
            NdZ = (/E%nx, E%ny, E%nz + 1/)
            !
        else
            stop "Error: printRVector > Only EDGE or FACE types allowed."
        endif
        !
        ...
        !
        write( funit, * ) E%nx, E%ny, E%nz
        write(funit, * ) "x-component",NdX
        do ix = 1, NdX(1)
             do iy = 1, NdX(2)
                do iz = 1, NdX(3)
                     if( E%x( ix, iy, iz ) /= 0 ) then
                        write(funit,*) ix,iy,iz, ":[", E%x( ix, iy, iz ), "]"
                     endif
                enddo
             enddo
        enddo
        !
        write(funit,*) "y-component",NdY
        do ix = 1, NdY(1)
             do iy = 1, NdY(2)
                do iz = 1, NdY(3)
                     if( E%y( ix, iy, iz ) /= 0 ) then
                        write(funit,*) ix,iy,iz, ":[", E%y( ix, iy, iz ), "]"
                     endif
                enddo
             enddo
        enddo
        !
        write(funit,*) "z-component",NdZ
        do ix = 1, NdZ(1)
             do iy = 1, NdZ(2)
                do iz = 1, NdZ(3)
                     if( E%z( ix, iy, iz ) /= 0 ) then
                        write(funit,*) ix,iy,iz, ":[", E%z( ix, iy, iz ), "]"
                     endif
                enddo
             enddo
        enddo
        !
    end subroutine printRVector
    !
    !> No subroutine briefing
    subroutine printCVector( E, io_unit, title )
        implicit none
        !
        class( cvector ), intent( in ) :: E
        integer, intent( in ), optional       :: io_unit
        character(*), intent( in ), optional  :: title
        !
        integer :: ix, iy, iz, funit
        ...
        if( E%gridType == EDGE ) then
			...
			write( *, * ) E%gridType
            stop "Error: printCVector > Only EDGE or FACE types allowed."
        endif
        !
        if( present( io_unit ) ) then
            funit = io_unit
