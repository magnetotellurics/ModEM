1. Getting Start.
	...
	1.1. Unzip ModEM-2.0.0/

		1.1.1. For Unix systems, just jump to 1.2.
	...
	1.3. Compiling/Installing ModEM.
		...
		1.3.1. Creating Makefiles and Compiling executables:
			...
			- To compile and create both ModEM_SERIAL and ModEM_MPI executables at once:

				bash ../scripts/compile_serial_mpi.sh

			- To create only ModEM_MPI:

				bash ../scripts/compile_mpi.sh

			- To create only ModEM_SERIAL:

				bash ../scripts/compile_serial.sh

			- To generate just a Makefile (Serial or MPI):

				- MakefileSerial -> ./Build/Configure.modem.ubuntu.gfortran MakefileSerial SERIAL ModEM.f90

				- MakefileMPI    -> ./Build/Configure.modem.ubuntu.gfortran MakefileMPI MPI ModEM.f90

			- To create only a ModEM executable (according to a Makefile):

				make -f <MAKEFILE>

			- [Mac OS only]

				- Serial -> make -f Build/MakefileSerial_MAC

				- MPI    -> make -f Build/MakefileMPI_MAC ????

		Note: Executables and Makefiles can be renamed at will.
...
2. Running ModEM.
	...
	Note: If everything was done correctly, minimal program usage instructions will appear on the screen.
	...
	2.2. ModEM Main Jobs.
		...
		- Different jobs are determined using specific flags (shown in 3.2.),
			which will direct the program to perform one of the following tasks:

		2.2.1. [-f]	Forward Modeling (FWD).

			- To perform FWD job using default parameters,
			...
			- This generates an output Data File, named 'all_predicted_data.dat',
				in the same format as the input one, containing the data predicted by the FWD.
		...
		2.2.2. [-i]	Inversion (INV).
			...
			- To perform INV job using default parameters,
				just Model and Data Files are needed, preceded by -m and -d flags respectively, as follows:
		...
		2.2.5. Control File Templates.
			...
			- FWD and INV control parameters can be configured through input text files.
				Each featuring its specific and editable variables, and can be created by running:

					./ModEM --template

			- This create two distinct text files named:

			2.2.5.1. fwd_ctrl_template.txt
...
4. More ????
	...
	4.1. Extensive Doxygen Source Code documentation can be found at:
