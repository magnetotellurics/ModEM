...
#
#
echo "#### COMMAND LINE: [mpirun -n $ncores ../$EXEC -M ../$MODEL ../$dMODEL ../$DATA wFile_Data -v full]" | tee -a std_out.txt
#
#
mpirun -n $ncores ../$EXEC -M ../$MODEL ../$dMODEL ../$DATA wFile_Data -v full | tee -a std_out.txt
#
# CATCH RESULT
result=$?
#
...
if [ "$result" -ne "0" ]; then
	#
	#
	echo "TEST MULT_BY_J FAIL: $result" | tee -a std_out.txt
	#
	#
	cd ..
	#
...
#
#
echo "#### FINISH MULT_BY_J MPI TEST ####" | tee -a std_out.txt
#
#
cd ..
#
