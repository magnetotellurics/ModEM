...
for DIR in $DIRS; do
    ...
    if [ -d "$DIR" ]; then
		...
		T_START=$(date +%s%3N)
		#
		echo "	> [${DIR_NAME}]: START TEST FORWARD ${EXEC_NAME} WITH $NCORES CORES" | tee -a outputs/temp/summary.txt
		#
		...
		if [ "$RESULT" -ne "0" ]; then
			#
			echo "	> [${DIR_NAME}]: TEST FORWARD ${EXEC_NAME} FAIL: $RESULT" | tee -a outputs/temp/summary.txt
			T_END=$(date +%s%3N)
		...
		#
		echo "	> [${DIR_NAME}]: TEST FORWARD ${EXEC_NAME} PASS: $RESULT" | tee -a outputs/temp/summary.txt
		T_END=$(date +%s%3N)
		echo "	> [${DIR_NAME}]: Time Spent: $(( ( $T_END - $T_START ) / 1000 )) seconds" | tee -a outputs/temp/summary.txt
		#
		echo "	> [${DIR_NAME}]: CREATE SolverDiagnostic3D" | tee -a outputs/temp/summary.txt
		#
		T_START=$(date +%s%3N)
		#
